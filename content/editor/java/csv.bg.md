...
############################# Head ############################
head_title: "CSV Editor — Редактирайте CSV в Java"
head_description: "Как да редактирате CSV в Java с помощта на няколко реда код? Използвайте API за обработка на документи на GroupDocs, за да редактирате, актуализирате и запазвате 30+ файлови формата."

############################# Header ############################
title: "Редактирайте CSV в Java"
description: "Ефективно и стабилно CSV редактиране с помощта на GroupDocs.Editor от страна на сървъра за API на Java, без използването на софтуер като Microsoft или Open Office."
...
steps:
    enable: true
    title_left: "Стъпки за редактиране на CSV в Java"
    content_left: |
        [GroupDocs.Editor for Java](/bg/editor/java/) предоставя лесен и ясен начин за разработчиците да редактират файловете CSV с помощта на няколко реда код.
        * Създайте екземпляр на клас „Редактор“ със задължителен файлов път или поток от байтове и заредете файла CSV
        * Създайте екземпляр на клас `DelimitedTextEditOptions` за файловия формат CSV и укажете задължителен разделител на низове в конструктора
        * Извикайте метода `Editor.Edit()` и получете CSV документ в HTML формат, който може лесно да се редактира с всеки WYSIWYG-редактор.
        * Извикайте метода `Editor.Save()` и запазете редактирания файл CSV, като използвате екземпляра на класа `DelimitedTextSaveOptions` с желания разделител
...
demos:
    enable: true
    title: "CSV Редакторски демонстрации на живо"
    content: |
        Редактирайте CSV точно сега, като посетите уебсайта [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
