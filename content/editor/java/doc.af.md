...
############################# Head ############################
head_title: "DOC-redigeerder — Wysig DOC in Java"
head_description: "Hoe om DOC in Java te wysig deur 'n paar reëls kode te gebruik? Gebruik GroupDocs-dokumente wat API's verwerk om 30+ lêerformate te redigeer, op te dateer en te stoor."

############################# Header ############################
title: "Wysig DOC in Java"
description: "Effektiewe en robuuste DOC redigering deur bedienerkant GroupDocs.Editor vir Java API's te gebruik, sonder die gebruik van enige sagteware soos Microsoft of Open Office."
...
steps:
    enable: true
    title_left: "Stappe om DOC in Java te wysig"
    content_left: |
        [GroupDocs.Editor for Java](/af/editor/java/) bied 'n maklike en reguit manier vir ontwikkelaars om die DOC lêers te wysig deur 'n paar reëls kode te gebruik.
        * Skep 'n instansie van `Editor` klas met verpligte lêerpad of stroom en opsionele `WordProcessingLoadOptions` klas en laai die DOC lêer
        * Skep en stel die `WordProcessingEditOptions`-klasinstansie vir die DOC-lêerformaat
        * Roep `Editor.Edit()`-metode en verkry DOC-dokument in HTML-formaat wat maklik met enige WYSIWYG-redigeerder geredigeer kan word.
        * Roep `Editor.Save()`-metode en stoor geredigeerde DOC-lêer met `WordProcessingSaveOptions`-klas
...
demos:
    enable: true
    title: "DOC Redigeerder Regstreekse Demo's"
    content: |
        Wysig DOC op die oomblik deur [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family) webwerf te besoek.
