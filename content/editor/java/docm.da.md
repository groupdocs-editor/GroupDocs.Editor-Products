...
head_description: "Hvordan redigerer man DOCM i Java ved hjælp af et par linjer kode? Brug GroupDocs-dokumenter, der behandler API'er til at redigere, opdatere og gemme mere end 30 filformater."

############################# Header ############################
title: "Rediger DOCM i Java"
description: "Effektiv og robust DOCM redigering ved hjælp af serverside GroupDocs.Editor til Java API'er, uden brug af software som Microsoft eller Open Office."
...
steps:
    enable: true
    title_left: "Trin til redigering af DOCM i Java"
    content_left: |
        [GroupDocs.Editor for Java](/da/editor/java/) giver en nem og ligetil måde for udviklere at redigere DOCM-filerne ved hjælp af et par linjer kode.
        * Opret en forekomst af klassen `Editor` med obligatorisk filsti eller stream og valgfri `WordProcessingLoadOptions` klasse og indlæs DOCM filen
        * Opret og indstil klasseforekomsten `WordProcessingEditOptions` for filformatet DOCM
        * Kald `Editor.Edit()`-metoden og få et DOCM-dokument i HTML-format, der nemt kan redigeres med enhver WYSIWYG-editor.
        * Kald `Editor.Save()`-metoden og gem den redigerede DOCM-fil ved hjælp af `WordProcessingSaveOptions`-klassen
...
demos:
    ...
    content: |
        Rediger DOCM lige nu ved at besøge webstedet [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
