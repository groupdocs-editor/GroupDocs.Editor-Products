...
############################# Head ############################
head_title: "Edytor DOCM — Edytuj DOCM w Java"
head_description: "Jak edytować DOCM w Java za pomocą kilku linijek kodu? Użyj interfejsów API przetwarzania dokumentów GroupDocs, aby edytować, aktualizować i zapisywać ponad 30 formatów plików."

############################# Header ############################
title: "Edytuj DOCM w Java"
description: "Efektywna i solidna edycja DOCM przy użyciu GroupDocs.Editor po stronie serwera dla interfejsów API Java, bez użycia jakiegokolwiek oprogramowania, takiego jak Microsoft lub Open Office."
...
steps:
    enable: true
    title_left: "Kroki edycji DOCM w Java"
    content_left: |
        [GroupDocs.Editor for Java](/pl/editor/java/) zapewnia programistom łatwy i prosty sposób edytowania plików DOCM przy użyciu kilku linijek kodu.
        * Utwórz instancję klasy `Editor` z obowiązkową ścieżką pliku lub strumieniem i opcjonalną klasą `WordProcessingLoadOptions` i załaduj plik DOCM
        * Utwórz i ustaw instancję klasy `WordProcessingEditOptions` dla formatu pliku DOCM
        * Wywołaj metodę `Editor.Edit()` i uzyskaj dokument DOCM w formacie HTML, który można łatwo edytować dowolnym edytorem WYSIWYG.
        * Wywołaj metodę `Editor.Save()` i zapisz edytowany plik DOCM przy użyciu klasy `WordProcessingSaveOptions`
...
demos:
    enable: true
    title: "DOCM Demonstracje na żywo dla edytora"
    content: |
        Edytuj DOCM już teraz, odwiedzając witrynę [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
