...
############################# Head ############################
head_title: "DOCX Eagarthóir — Cuir DOCX in eagar in {{ PlatformName}}"
head_description: "Conas DOCX in Java a chur in eagar ag úsáid cúpla líne de chód? Bain úsáid as APIanna próiseála doiciméad GroupDocs chun 30+ formáid comhaid a chur in eagar, a nuashonrú agus a shábháil."

############################# Header ############################
title: "Cuir DOCX in eagar in {{ PlatformName}}"
description: "Eagarthóireacht éifeachtach agus láidir DOCX ag baint úsáide as taobh an fhreastalaí GroupDocs.Editor le haghaidh APIanna {{ PlatformName}}, gan úsáid a bhaint as aon bhogearraí ar nós Microsoft nó Open Office."
...
steps:
    enable: true
    title_left: "Céimeanna chun DOCX a chur in eagar in Java"
    content_left: |
        Soláthraíonn [GroupDocs.Editor for Java](/ga/editor/java/) bealach éasca agus simplí d’fhorbróirí chun na comhaid DOCX a chur in eagar ag úsáid cúpla líne cód.
        * Cruthaigh sampla de rang `Eagarthóir` le conair nó sruth comhaid éigeantach agus rang roghnach `WordProcessingLoadOptions` agus luchtaigh an comhad DOCX
        * Cruthaigh & socraigh an sampla ranga `WordProcessingEditOptions` don fhormáid comhaid DOCX
        * Glaoigh ar an modh `Editor.Edit()` agus faigh doiciméad DOCX i bhformáid HTML is furasta a chur in eagar le haon eagarthóir WYSIWYG.
        * Glaoigh ar an modh `Editor.Save()` agus sábháil an comhad DOCX atá curtha in eagar ag úsáid ranga `WordProcessingSaveOptions`
...
demos:
    enable: true
    title: "DOCX Eagarthóir Beo Taispeántais"
    content: |
        Cuir DOCX in eagar faoi láthair trí chuairt a thabhairt ar [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
