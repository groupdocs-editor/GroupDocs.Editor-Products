...
############################# Head ############################
head_title: "DOCX 편집기 — Java에서 DOCX 편집"
head_description: "몇 줄의 코드를 사용하여 Java에서 DOCX를 편집하는 방법은 무엇입니까? GroupDocs 문서 처리 API를 사용하여 30개 이상의 파일 형식을 편집, 업데이트 및 저장합니다."

############################# Header ############################
title: "Java에서 DOCX 수정"
description: "Microsoft 또는 Open Office와 같은 소프트웨어를 사용하지 않고 Java API용 서버 측 GroupDocs.Editor를 사용하여 효과적이고 강력한 DOCX 편집."
...
steps:
    enable: true
    title_left: "Java에서 DOCX를 수정하는 단계"
    content_left: |
        [GroupDocs.Editor for Java](/ko/editor/java/)는 개발자가 몇 줄의 코드를 사용하여 DOCX 파일을 편집할 수 있는 쉽고 간단한 방법을 제공합니다.
        * 필수 파일 경로 또는 스트림과 선택적 `WordProcessingLoadOptions` 클래스가 있는 `Editor` 클래스의 인스턴스를 만들고 DOCX 파일을 로드합니다.
        * DOCX 파일 형식에 대한 `WordProcessingEditOptions` 클래스 인스턴스 생성 및 설정
        * `Editor.Edit()` 메서드를 호출하고 모든 WYSIWYG 편집기로 쉽게 편집할 수 있는 HTML 형식의 DOCX 문서를 가져옵니다.
        * `Editor.Save()` 메서드를 호출하고 `WordProcessingSaveOptions` 클래스를 사용하여 편집된 DOCX 파일을 저장합니다.
...
demos:
    enable: true
    title: "DOCX 편집자 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Editor 라이브 데모](https://products.groupdocs.app/editor/family) 웹사이트를 방문하여 DOCX을(를) 편집하세요.
