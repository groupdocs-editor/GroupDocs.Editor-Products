...
draft: false
otherformats: doc docm dotx xls xlsx xlsm ppt pptx pptm mobi epub html mhtml txt xml csv pdf xps
...
steps:
    enable: true
    title_left: "Кроки по редагуванню DOCX на Java"
    content_left: |
        [GroupDocs.Editor for Java](/uk/editor/java/) надає розробникам простий і зрозумілий спосіб редагування файлів DOCX за допомогою кількох рядків коду.
        * Створіть екземпляр класу `Editor` із вказанням обов'язкового шляху до файлу чи потоки та необов'язковим класом `WordProcessingLoadOptions` і завантажте файл DOCX
        * Створіть та налаштуйте екземпляр класу `WordProcessingEditOptions` для формату файлу DOCX
        * Викличте метод `Editor.Edit()` і отримайте документ DOCX у форматі HTML, котрий легко редагується любим WYSIWYG-редактором.
        * Викличте метод `Editor.Save()` і збережіть відредагований файл DOCX, використовуючи клас `WordProcessingSaveOptions`
...
demos:
    enable: true
    title: "Демоверсії редактора DOCX"
    content: |
        Відредагуйте DOCX прямо зараз, відвідав [живі демонстрації](https://products.groupdocs.app/editor/family). Жива демонстрація має наступні переваги
