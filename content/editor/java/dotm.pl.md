...
############################# Head ############################
head_title: "Edytor DOTM — Edytuj DOTM w Java"
head_description: "Jak edytować DOTM w Java za pomocą kilku linijek kodu? Użyj interfejsów API przetwarzania dokumentów GroupDocs, aby edytować, aktualizować i zapisywać ponad 30 formatów plików."

############################# Header ############################
title: "Edytuj DOTM w Java"
description: "Efektywna i solidna edycja DOTM przy użyciu GroupDocs.Editor po stronie serwera dla interfejsów API Java, bez użycia jakiegokolwiek oprogramowania, takiego jak Microsoft lub Open Office."
...
steps:
    enable: true
    title_left: "Kroki edycji DOTM w Java"
    content_left: |
        [GroupDocs.Editor for Java](/pl/editor/java/) zapewnia programistom łatwy i prosty sposób edytowania plików DOTM przy użyciu kilku linijek kodu.
        * Utwórz instancję klasy `Editor` z obowiązkową ścieżką pliku lub strumieniem i opcjonalną klasą `WordProcessingLoadOptions` i załaduj plik DOTM
        * Utwórz i ustaw instancję klasy `WordProcessingEditOptions` dla formatu pliku DOTM
        * Wywołaj metodę `Editor.Edit()` i uzyskaj dokument DOTM w formacie HTML, który można łatwo edytować dowolnym edytorem WYSIWYG.
        * Wywołaj metodę `Editor.Save()` i zapisz edytowany plik DOTM przy użyciu klasy `WordProcessingSaveOptions`
...
demos:
    enable: true
    title: "DOTM Demonstracje na żywo dla edytora"
    content: |
        Edytuj DOTM już teraz, odwiedzając witrynę [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
