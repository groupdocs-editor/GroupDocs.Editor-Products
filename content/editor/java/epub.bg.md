...
############################# Head ############################
head_title: "EPUB Editor — Редактирайте EPUB в Java"
head_description: "Как да редактирате EPUB в Java с помощта на няколко реда код? Използвайте API за обработка на документи на GroupDocs, за да редактирате, актуализирате и запазвате 30+ файлови формата."

############################# Header ############################
title: "Редактирайте EPUB в Java"
description: "Ефективно и стабилно EPUB редактиране с помощта на GroupDocs.Editor от страна на сървъра за API на Java, без използването на софтуер като Microsoft или Open Office."
...
steps:
    enable: true
    title_left: "Стъпки за редактиране на EPUB в Java"
    content_left: |
        [GroupDocs.Editor for Java](/bg/editor/java/) предоставя лесен и ясен начин за разработчиците да редактират файловете EPUB с помощта на няколко реда код.
        * Създайте екземпляр на клас „Редактор“ със задължителен файлов път или поток от байтове и заредете файла EPUB
        * Създайте и задайте екземпляр на класа `EbookEditOptions` за файловия формат EPUB
        * Извикайте метода `Editor.Edit()` и получете EPUB документ в HTML формат, който може лесно да се редактира с всеки WYSIWYG-редактор.
        * Извикайте метода `Editor.Save()` и запазете редактирания EPUB файл, като използвате класа `EpubSaveOptions` или `Azw3SaveOptions` за запазване съответно в ePub или AZW3 формати
...
demos:
    enable: true
    title: "EPUB Редакторски демонстрации на живо"
    content: |
        Редактирайте EPUB точно сега, като посетите уебсайта [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
