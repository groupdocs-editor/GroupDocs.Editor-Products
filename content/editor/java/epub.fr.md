...
############################# Head ############################
head_title: "Éditeur EPUB : modifiez EPUB dans Java"
head_description: "Comment modifier EPUB dans Java en utilisant quelques lignes de code ? Utilisez les API de traitement de documents GroupDocs pour modifier, mettre à jour et enregistrer plus de 30 formats de fichiers."

############################# Header ############################
title: "Modifier EPUB dans Java"
description: "Édition EPUB efficace et robuste à l'aide de GroupDocs.Editor côté serveur pour les API Java, sans l'utilisation de logiciels tels que Microsoft ou Open Office."
...
steps:
    enable: true
    title_left: "Étapes pour modifier EPUB dans Java"
    content_left: |
        [GroupDocs.Editor for Java](/fr/editor/java/) offre aux développeurs un moyen simple et direct de modifier les fichiers EPUB à l'aide de quelques lignes de code.
        * Créez une instance de la classe `Editor` avec un chemin de fichier obligatoire ou un flux d'octets et chargez le fichier EPUB
        * Créez et définissez l'instance de classe `EbookEditOptions` pour le format de fichier EPUB
        * Appelez la méthode `Editor.Edit()` et obtenez le document EPUB au format HTML facilement modifiable avec n'importe quel éditeur WYSIWYG.
        * Appelez la méthode `Editor.Save()` et enregistrez le fichier EPUB modifié à l'aide de la classe `EpubSaveOptions` ou `Azw3SaveOptions` pour l'enregistrement aux formats ePub ou AZW3 respectivement
...
demos:
    enable: true
    title: "EPUB démos en direct de l'éditeur"
    content: |
        Modifiez EPUB dès maintenant en visitant le site Web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
