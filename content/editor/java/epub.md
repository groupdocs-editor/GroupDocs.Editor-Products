...
############################# Head ############################
head_title: "EPUB Editor — Edit EPUB in Java"
head_description: "How to edit EPUB in Java using a few lines of code? Use GroupDocs documents processing APIs to edit, update and save 30+ file formats."

############################# Header ############################
title: "Edit EPUB in Java"
description: "Effective and robust EPUB editing using server side GroupDocs.Editor for Java APIs, without the use of any software like Microsoft or Open Office."
...
steps:
    enable: true
    title_left: "Steps to Edit EPUB in Java"
    content_left: |
        [GroupDocs.Editor for Java](/editor/java/) provides an easy and straightforward way for developers to edit the EPUB files using a few lines of code.
        * Create an instance of `Editor` class with mandatory file path or byte stream and load the EPUB file
        * Create & set the `EbookEditOptions` class instance for the EPUB file format
        * Call `Editor.Edit()` method and obtain EPUB document in HTML format that is easily editable with any WYSIWYG-editor.
        * Call `Editor.Save()` method and save edited EPUB file using `EpubSaveOptions` or `Azw3SaveOptions` class for the saving into ePub or AZW3 formats respectively
...
demos:
    ...
    content: |
        Edit EPUB right now by visiting [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family) website.  
