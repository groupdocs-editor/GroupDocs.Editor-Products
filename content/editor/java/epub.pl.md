...
############################# Head ############################
head_title: "Edytor EPUB — Edytuj EPUB w Java"
head_description: "Jak edytować EPUB w Java za pomocą kilku linijek kodu? Użyj interfejsów API przetwarzania dokumentów GroupDocs, aby edytować, aktualizować i zapisywać ponad 30 formatów plików."

############################# Header ############################
title: "Edytuj EPUB w Java"
description: "Efektywna i solidna edycja EPUB przy użyciu GroupDocs.Editor po stronie serwera dla interfejsów API Java, bez użycia jakiegokolwiek oprogramowania, takiego jak Microsoft lub Open Office."
...
steps:
    enable: true
    title_left: "Kroki edycji EPUB w Java"
    content_left: |
        [GroupDocs.Editor for Java](/pl/editor/java/) zapewnia programistom łatwy i prosty sposób edytowania plików EPUB przy użyciu kilku linijek kodu.
        * Utwórz instancję klasy `Editor` z obowiązkową ścieżką pliku lub strumieniem bajtów i załaduj plik EPUB
        * Utwórz i ustaw instancję klasy `EbookEditOptions` dla formatu pliku EPUB
        * Wywołaj metodę `Editor.Edit()` i uzyskaj dokument EPUB w formacie HTML, który można łatwo edytować dowolnym edytorem WYSIWYG.
        * Wywołaj metodę `Editor.Save()` i zapisz edytowany plik EPUB przy użyciu klasy `EpubSaveOptions` lub `Azw3SaveOptions` w celu zapisania odpowiednio w formatach ePub lub AZW3
...
demos:
    enable: true
    title: "EPUB Demonstracje na żywo dla edytora"
    content: |
        Edytuj EPUB już teraz, odwiedzając witrynę [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
