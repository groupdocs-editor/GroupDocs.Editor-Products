...
############################# Head ############################
head_title: "HTM Editor - Chỉnh sửa HTM trong Java"
head_description: "Làm cách nào để chỉnh sửa HTM trong Java bằng một vài dòng mã? Sử dụng API xử lý tài liệu GroupDocs để chỉnh sửa, cập nhật và lưu hơn 30 định dạng tệp."

############################# Header ############################
title: "Chỉnh sửa HTM trong Java"
description: "Chỉnh sửa HTM hiệu quả và mạnh mẽ bằng cách sử dụng GroupDocs.Editor phía máy chủ cho các API Java mà không cần sử dụng bất kỳ phần mềm nào như Microsoft hoặc Open Office."
...
steps:
    enable: true
    title_left: "Các bước chỉnh sửa HTM trong Java"
    content_left: |
        [GroupDocs.Editor for Java] (/vi / editor / java /) cung cấp một cách dễ dàng và đơn giản để nhà phát triển chỉnh sửa tệp HTM bằng một vài dòng mã.
        * Tạo một phiên bản của lớp `Editor` với đường dẫn tệp bắt buộc hoặc luồng byte và tải tệp HTM
        * Tạo và đặt phiên bản lớp `TextEditOptions` cho định dạng tệp HTM
        * Gọi phương thức `Editor.Edit ()` và lấy tài liệu HTM ở định dạng HTML có thể chỉnh sửa dễ dàng bằng bất kỳ trình soạn thảo WYSIWYG nào.
        * Gọi phương thức `Editor.Save ()` và lưu tệp HTM đã chỉnh sửa bằng cách sử dụng lớp `TextSaveOptions`
...
demos:
    enable: true
    title: "Bản trình diễn trực tiếp của HTM Editor"
    content: |
        Chỉnh sửa HTM ngay bây giờ bằng cách truy cập trang web [GroupDocs.Editor Live Demos] (https://products.groupdocs.app/editor/family) trang web.
