...
############################# Head ############################
head_title: "HTML Eagarthóir — Cuir HTML in eagar in {{ PlatformName}}"
head_description: "Conas HTML in Java a chur in eagar ag úsáid cúpla líne de chód? Bain úsáid as APIanna próiseála doiciméad GroupDocs chun 30+ formáid comhaid a chur in eagar, a nuashonrú agus a shábháil."

############################# Header ############################
title: "Cuir HTML in eagar in {{ PlatformName}}"
description: "Eagarthóireacht éifeachtach agus láidir HTML ag baint úsáide as taobh an fhreastalaí GroupDocs.Editor le haghaidh APIanna {{ PlatformName}}, gan úsáid a bhaint as aon bhogearraí ar nós Microsoft nó Open Office."
...
steps:
    enable: true
    title_left: "Céimeanna chun HTML a chur in eagar in Java"
    content_left: |
        Soláthraíonn [GroupDocs.Editor for Java](/ga/editor/java/) bealach éasca agus simplí d’fhorbróirí chun na comhaid HTML a chur in eagar ag úsáid cúpla líne cód.
        * Cruthaigh sampla de rang `Eagarthóir` le conair éigeantach comhaid nó sruth beart agus lódáil an comhad HTML
        * Cruthaigh & socraigh an sampla ranga `TextEditOptions` don fhormáid comhaid HTML
        * Glaoigh ar an modh `Editor.Edit()` agus faigh doiciméad HTML i bhformáid HTML is furasta a chur in eagar le haon eagarthóir WYSIWYG.
        * Glaoigh ar an modh `Editor.Save()` agus sábháil an comhad HTML atá curtha in eagar ag úsáid rang `TextSaveOptions`
...
demos:
    enable: true
    title: "HTML Eagarthóir Beo Taispeántais"
    content: |
        Cuir HTML in eagar faoi láthair trí chuairt a thabhairt ar [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
