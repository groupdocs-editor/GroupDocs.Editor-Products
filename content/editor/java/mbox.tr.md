...
############################# Head ############################
head_title: "MBOX Düzenleyici — MBOX öğesini Java içinde düzenleyin"
head_description: "MBOX, birkaç satır kod kullanarak Java içinde nasıl düzenlenir? 30'dan fazla dosya biçimini düzenlemek, güncellemek ve kaydetmek için API'leri işleyen GroupDocs belgelerini kullanın."

############################# Header ############################
title: "MBOX öğesini Java içinde düzenleyin"
description: "Microsoft veya Open Office gibi herhangi bir yazılım kullanmadan Java API'leri için sunucu tarafı GroupDocs.Editor kullanarak etkili ve sağlam MBOX düzenleme."
...
steps:
    enable: true
    title_left: "Java içinde MBOX Düzenleme Adımları"
    content_left: |
        [GroupDocs.Editor for Java](/tr/editor/java/), geliştiricilerin birkaç satır kod kullanarak MBOX dosyalarını düzenlemeleri için kolay ve anlaşılır bir yol sağlar.
        * Zorunlu dosya yolu veya bayt akışıyla bir "Editör" sınıfı örneği oluşturun ve MBOX dosyasını yükleyin
        * MBOX dosya biçimi için "EmailEditOptions" sınıf örneğini oluşturun ve ayarlayın
        * `Editor.Edit()` yöntemini çağırın ve herhangi bir WYSIWYG editörüyle kolayca düzenlenebilen HTML formatında MBOX belgesini alın.
        * `Editor.Save()` yöntemini çağırın ve `EmailSaveOptions` sınıfını kullanarak düzenlenmiş MBOX dosyasını kaydedin
...
demos:
    enable: true
    title: "MBOX Editör Canlı Demoları"
    content: |
        MBOX dosyasını hemen [GroupDocs.Editor Canlı Demoları](https://products.groupdocs.app/editor/family) web sitesini ziyaret ederek düzenleyin.
