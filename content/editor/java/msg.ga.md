...
############################# Head ############################
head_title: "MSG Eagarthóir — Cuir MSG in eagar in {{ PlatformName}}"
head_description: "Conas MSG in Java a chur in eagar ag úsáid cúpla líne de chód? Bain úsáid as APIanna próiseála doiciméad GroupDocs chun 30+ formáid comhaid a chur in eagar, a nuashonrú agus a shábháil."

############################# Header ############################
title: "Cuir MSG in eagar in {{ PlatformName}}"
description: "Eagarthóireacht éifeachtach agus láidir MSG ag baint úsáide as taobh an fhreastalaí GroupDocs.Editor le haghaidh APIanna {{ PlatformName}}, gan úsáid a bhaint as aon bhogearraí ar nós Microsoft nó Open Office."
...
steps:
    enable: true
    title_left: "Céimeanna chun MSG a chur in eagar in Java"
    content_left: |
        Soláthraíonn [GroupDocs.Editor for Java](/ga/editor/java/) bealach éasca agus simplí d’fhorbróirí chun na comhaid MSG a chur in eagar ag úsáid cúpla líne cód.
        * Cruthaigh sampla de rang `Eagarthóir` le conair éigeantach comhaid nó sruth beart agus lódáil an comhad MSG
        * Cruthaigh agus socraigh an sampla ranga `EmailEditOptions` don fhormáid comhaid MSG
        * Glaoigh ar an modh `Editor.Edit()` agus faigh doiciméad MSG i bhformáid HTML is furasta a chur in eagar le haon eagarthóir WYSIWYG.
        * Glaoigh ar an modh `Editor.Save()` agus sábháil an comhad MSG atá curtha in eagar ag úsáid rang `EmailSaveOptions`
...
demos:
    enable: true
    title: "MSG Eagarthóir Beo Taispeántais"
    content: |
        Cuir MSG in eagar faoi láthair trí chuairt a thabhairt ar [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
