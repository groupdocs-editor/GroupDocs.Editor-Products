...
############################# Head ############################
head_title: "ODP Eagarthóir — Cuir ODP in eagar in {{ PlatformName}}"
head_description: "Conas ODP in Java a chur in eagar ag úsáid cúpla líne de chód? Bain úsáid as APIanna próiseála doiciméad GroupDocs chun 30+ formáid comhaid a chur in eagar, a nuashonrú agus a shábháil."

############################# Header ############################
title: "Cuir ODP in eagar in {{ PlatformName}}"
description: "Eagarthóireacht éifeachtach agus láidir ODP ag baint úsáide as taobh an fhreastalaí GroupDocs.Editor le haghaidh APIanna {{ PlatformName}}, gan úsáid a bhaint as aon bhogearraí ar nós Microsoft nó Open Office."
...
steps:
    enable: true
    title_left: "Céimeanna chun ODP a chur in eagar in Java"
    content_left: |
        Soláthraíonn [GroupDocs.Editor for Java](/ga/editor/java/) bealach éasca agus simplí d’fhorbróirí chun na comhaid ODP a chur in eagar ag úsáid cúpla líne cód.
        * Cruthaigh sampla de rang `Eagarthóir` le conair éigeantach comhaid nó sruth beart agus rang roghnach `PresentationLoadOptions` agus lódáil an comhad ODP
        * Cruthaigh & socraigh an sampla ranga `PresentationEditOptions` don fhormáid comhaid ODP
        * Glaoigh ar an modh `Editor.Edit()` agus faigh doiciméad ODP i bhformáid HTML is furasta a chur in eagar le haon eagarthóir WYSIWYG.
        * Glaoigh ar an modh `Editor.Save()` agus sábháil an comhad ODP atá curtha in eagar ag úsáid rang `PresentationSaveOptions`
...
demos:
    enable: true
    title: "ODP Eagarthóir Beo Taispeántais"
    content: |
        Cuir ODP in eagar faoi láthair trí chuairt a thabhairt ar [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
