...
############################# Head ############################
head_title: "Edytor ODP — Edytuj ODP w Java"
head_description: "Jak edytować ODP w Java za pomocą kilku linijek kodu? Użyj interfejsów API przetwarzania dokumentów GroupDocs, aby edytować, aktualizować i zapisywać ponad 30 formatów plików."

############################# Header ############################
title: "Edytuj ODP w Java"
description: "Efektywna i solidna edycja ODP przy użyciu GroupDocs.Editor po stronie serwera dla interfejsów API Java, bez użycia jakiegokolwiek oprogramowania, takiego jak Microsoft lub Open Office."
...
steps:
    enable: true
    title_left: "Kroki edycji ODP w Java"
    content_left: |
        [GroupDocs.Editor for Java](/pl/editor/java/) zapewnia programistom łatwy i prosty sposób edytowania plików ODP przy użyciu kilku linijek kodu.
        * Utwórz instancję klasy `Editor` z obowiązkową ścieżką pliku lub strumieniem bajtów i opcjonalną klasą `PresentationLoadOptions` i załaduj plik ODP
        * Utwórz i ustaw instancję klasy `PresentationEditOptions` dla formatu pliku ODP
        * Wywołaj metodę `Editor.Edit()` i uzyskaj dokument ODP w formacie HTML, który można łatwo edytować dowolnym edytorem WYSIWYG.
        * Wywołaj metodę `Editor.Save()` i zapisz edytowany plik ODP przy użyciu klasy `PresentationSaveOptions`
...
demos:
    enable: true
    title: "ODP Demonstracje na żywo dla edytora"
    content: |
        Edytuj ODP już teraz, odwiedzając witrynę [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
