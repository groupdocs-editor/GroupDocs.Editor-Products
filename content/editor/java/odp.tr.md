...
############################# Head ############################
head_title: "ODP Düzenleyici — ODP öğesini Java içinde düzenleyin"
head_description: "ODP, birkaç satır kod kullanarak Java içinde nasıl düzenlenir? 30'dan fazla dosya biçimini düzenlemek, güncellemek ve kaydetmek için API'leri işleyen GroupDocs belgelerini kullanın."

############################# Header ############################
title: "ODP öğesini Java içinde düzenleyin"
description: "Microsoft veya Open Office gibi herhangi bir yazılım kullanmadan Java API'leri için sunucu tarafı GroupDocs.Editor kullanarak etkili ve sağlam ODP düzenleme."
...
steps:
    enable: true
    title_left: "Java içinde ODP Düzenleme Adımları"
    content_left: |
        [GroupDocs.Editor for Java](/tr/editor/java/), geliştiricilerin birkaç satır kod kullanarak ODP dosyalarını düzenlemeleri için kolay ve anlaşılır bir yol sağlar.
        * Zorunlu dosya yolu veya bayt akışı ve isteğe bağlı "PresentationLoadOptions" sınıfı ile bir "Editor" sınıfı örneği oluşturun ve ODP dosyasını yükleyin
        * ODP dosya biçimi için "PresentationEditOptions" sınıf örneğini oluşturun ve ayarlayın
        * `Editor.Edit()` yöntemini çağırın ve herhangi bir WYSIWYG editörüyle kolayca düzenlenebilen HTML formatında ODP belgesini alın.
        * `Editor.Save()` yöntemini çağırın ve düzenlenen ODP dosyasını `PresentationSaveOptions` sınıfını kullanarak kaydedin
...
demos:
    enable: true
    title: "ODP Editör Canlı Demoları"
    content: |
        ODP dosyasını hemen [GroupDocs.Editor Canlı Demoları](https://products.groupdocs.app/editor/family) web sitesini ziyaret ederek düzenleyin.
