...
############################# Head ############################
head_title: "Editor ODS — Editeu ODS a Java"
head_description: "Com editar ODS a Java amb unes quantes línies de codi? Utilitzeu les API de processament de documents de GroupDocs per editar, actualitzar i desar més de 30 formats de fitxer."

############################# Header ############################
title: "Editeu ODS a Java"
description: "Edició ODS eficaç i robusta mitjançant GroupDocs.Editor del servidor per a les API Java, sense utilitzar cap programari com Microsoft o Open Office."
...
steps:
    enable: true
    title_left: "Passos per editar ODS a Java"
    content_left: |
        [GroupDocs.Editor for Java](/ca/editor/java/) ofereix als desenvolupadors una manera senzilla i senzilla d'editar els fitxers ODS utilitzant unes poques línies de codi.
        * Creeu una instància de la classe `Editor` amb la ruta o flux del fitxer obligatori i la classe `SpreadsheetLoadOptions` opcional i carregueu el fitxer ODS
        * Creeu i configureu la instància de classe `SpreadsheetEditOptions` per al format de fitxer ODS
        * Truqueu al mètode `Editor.Edit()` i obteniu el document ODS en format HTML que es pugui editar fàcilment amb qualsevol editor WYSIWYG.
        * Truqueu al mètode `Editor.Save()` i deseu el fitxer editat ODS mitjançant la classe `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "ODS Editor de demostracions en directe"
    content: |
        Editeu ODS ara mateix visitant el lloc web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
