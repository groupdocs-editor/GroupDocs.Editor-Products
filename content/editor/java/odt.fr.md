...
############################# Head ############################
head_title: "Éditeur ODT : modifiez ODT dans Java"
head_description: "Comment modifier ODT dans Java en utilisant quelques lignes de code ? Utilisez les API de traitement de documents GroupDocs pour modifier, mettre à jour et enregistrer plus de 30 formats de fichiers."

############################# Header ############################
title: "Modifier ODT dans Java"
description: "Édition ODT efficace et robuste à l'aide de GroupDocs.Editor côté serveur pour les API Java, sans l'utilisation de logiciels tels que Microsoft ou Open Office."
...
steps:
    enable: true
    title_left: "Étapes pour modifier ODT dans Java"
    content_left: |
        [GroupDocs.Editor for Java](/fr/editor/java/) offre aux développeurs un moyen simple et direct de modifier les fichiers ODT à l'aide de quelques lignes de code.
        * Créez une instance de la classe `Editor` avec un chemin de fichier ou un flux obligatoire et une classe facultative `WordProcessingLoadOptions` et chargez le fichier ODT
        * Créez et définissez l'instance de classe `WordProcessingEditOptions` pour le format de fichier ODT
        * Appelez la méthode `Editor.Edit()` et obtenez le document ODT au format HTML facilement modifiable avec n'importe quel éditeur WYSIWYG.
        * Appelez la méthode `Editor.Save()` et enregistrez le fichier ODT modifié à l'aide de la classe `WordProcessingSaveOptions`
...
demos:
    enable: true
    title: "ODT démos en direct de l'éditeur"
    content: |
        Modifiez ODT dès maintenant en visitant le site Web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
