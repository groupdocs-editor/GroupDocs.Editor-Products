...
############################# Head ############################
head_title: "Edytor ODT — Edytuj ODT w Java"
head_description: "Jak edytować ODT w Java za pomocą kilku linijek kodu? Użyj interfejsów API przetwarzania dokumentów GroupDocs, aby edytować, aktualizować i zapisywać ponad 30 formatów plików."

############################# Header ############################
title: "Edytuj ODT w Java"
description: "Efektywna i solidna edycja ODT przy użyciu GroupDocs.Editor po stronie serwera dla interfejsów API Java, bez użycia jakiegokolwiek oprogramowania, takiego jak Microsoft lub Open Office."
...
steps:
    enable: true
    title_left: "Kroki edycji ODT w Java"
    content_left: |
        [GroupDocs.Editor for Java](/pl/editor/java/) zapewnia programistom łatwy i prosty sposób edytowania plików ODT przy użyciu kilku linijek kodu.
        * Utwórz instancję klasy `Editor` z obowiązkową ścieżką pliku lub strumieniem i opcjonalną klasą `WordProcessingLoadOptions` i załaduj plik ODT
        * Utwórz i ustaw instancję klasy `WordProcessingEditOptions` dla formatu pliku ODT
        * Wywołaj metodę `Editor.Edit()` i uzyskaj dokument ODT w formacie HTML, który można łatwo edytować dowolnym edytorem WYSIWYG.
        * Wywołaj metodę `Editor.Save()` i zapisz edytowany plik ODT przy użyciu klasy `WordProcessingSaveOptions`
...
demos:
    enable: true
    title: "ODT Demonstracje na żywo dla edytora"
    content: |
        Edytuj ODT już teraz, odwiedzając witrynę [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
