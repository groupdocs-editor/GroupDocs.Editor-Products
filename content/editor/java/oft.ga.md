...
############################# Head ############################
head_title: "OFT Eagarthóir — Cuir OFT in eagar in {{ PlatformName}}"
head_description: "Conas OFT in Java a chur in eagar ag úsáid cúpla líne de chód? Bain úsáid as APIanna próiseála doiciméad GroupDocs chun 30+ formáid comhaid a chur in eagar, a nuashonrú agus a shábháil."

############################# Header ############################
title: "Cuir OFT in eagar in {{ PlatformName}}"
description: "Eagarthóireacht éifeachtach agus láidir OFT ag baint úsáide as taobh an fhreastalaí GroupDocs.Editor le haghaidh APIanna {{ PlatformName}}, gan úsáid a bhaint as aon bhogearraí ar nós Microsoft nó Open Office."
...
steps:
    enable: true
    title_left: "Céimeanna chun OFT a chur in eagar in Java"
    content_left: |
        Soláthraíonn [GroupDocs.Editor for Java](/ga/editor/java/) bealach éasca agus simplí d’fhorbróirí chun na comhaid OFT a chur in eagar ag úsáid cúpla líne cód.
        * Cruthaigh sampla de rang `Eagarthóir` le conair éigeantach comhaid nó sruth beart agus lódáil an comhad OFT
        * Cruthaigh agus socraigh an sampla ranga `EmailEditOptions` don fhormáid comhaid OFT
        * Glaoigh ar an modh `Editor.Edit()` agus faigh doiciméad OFT i bhformáid HTML is furasta a chur in eagar le haon eagarthóir WYSIWYG.
        * Glaoigh ar an modh `Editor.Save()` agus sábháil an comhad OFT atá curtha in eagar ag úsáid rang `EmailSaveOptions`
...
demos:
    enable: true
    title: "OFT Eagarthóir Beo Taispeántais"
    content: |
        Cuir OFT in eagar faoi láthair trí chuairt a thabhairt ar [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
