...
############################# Head ############################
head_title: "OST Eagarthóir — Cuir OST in eagar in {{ PlatformName}}"
head_description: "Conas OST in Java a chur in eagar ag úsáid cúpla líne de chód? Bain úsáid as APIanna próiseála doiciméad GroupDocs chun 30+ formáid comhaid a chur in eagar, a nuashonrú agus a shábháil."

############################# Header ############################
title: "Cuir OST in eagar in {{ PlatformName}}"
description: "Eagarthóireacht éifeachtach agus láidir OST ag baint úsáide as taobh an fhreastalaí GroupDocs.Editor le haghaidh APIanna {{ PlatformName}}, gan úsáid a bhaint as aon bhogearraí ar nós Microsoft nó Open Office."
...
steps:
    enable: true
    title_left: "Céimeanna chun OST a chur in eagar in Java"
    content_left: |
        Soláthraíonn [GroupDocs.Editor for Java](/ga/editor/java/) bealach éasca agus simplí d’fhorbróirí chun na comhaid OST a chur in eagar ag úsáid cúpla líne cód.
        * Cruthaigh sampla de rang `Eagarthóir` le conair éigeantach comhaid nó sruth beart agus lódáil an comhad OST
        * Cruthaigh agus socraigh an sampla ranga `EmailEditOptions` don fhormáid comhaid OST
        * Glaoigh ar an modh `Editor.Edit()` agus faigh doiciméad OST i bhformáid HTML is furasta a chur in eagar le haon eagarthóir WYSIWYG.
        * Glaoigh ar an modh `Editor.Save()` agus sábháil an comhad OST atá curtha in eagar ag úsáid rang `EmailSaveOptions`
...
demos:
    enable: true
    title: "OST Eagarthóir Beo Taispeántais"
    content: |
        Cuir OST in eagar faoi láthair trí chuairt a thabhairt ar [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
