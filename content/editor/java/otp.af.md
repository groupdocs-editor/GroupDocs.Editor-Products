...
############################# Head ############################
head_title: "OTP-redigeerder — Wysig OTP in Java"
head_description: "Hoe om OTP in Java te wysig deur 'n paar reëls kode te gebruik? Gebruik GroupDocs-dokumente wat API's verwerk om 30+ lêerformate te redigeer, op te dateer en te stoor."

############################# Header ############################
title: "Wysig OTP in Java"
description: "Effektiewe en robuuste OTP redigering deur bedienerkant GroupDocs.Editor vir Java API's te gebruik, sonder die gebruik van enige sagteware soos Microsoft of Open Office."
...
steps:
    enable: true
    title_left: "Stappe om OTP in Java te wysig"
    content_left: |
        [GroupDocs.Editor for Java](/af/editor/java/) bied 'n maklike en reguit manier vir ontwikkelaars om die OTP lêers te wysig deur 'n paar reëls kode te gebruik.
        * Skep 'n instansie van `Editor`-klas met verpligte lêerpad of greepstroom en opsionele `PresentationLoadOptions`-klas en laai die OTP-lêer
        * Skep en stel die `PresentationEditOptions`-klasinstansie vir die OTP-lêerformaat
        * Roep `Editor.Edit()`-metode en verkry OTP-dokument in HTML-formaat wat maklik met enige WYSIWYG-redigeerder geredigeer kan word.
        * Roep `Editor.Save()`-metode en stoor geredigeerde OTP-lêer deur gebruik te maak van `PresentationSaveOptions`-klas
...
demos:
    enable: true
    title: "OTP Redigeerder Regstreekse Demo's"
    content: |
        Wysig OTP op die oomblik deur [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family) webwerf te besoek.
