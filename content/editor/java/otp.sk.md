...
############################# Head ############################
head_title: "Editor OTP – Upravte OTP v Java"
head_description: "Ako upraviť OTP v Java pomocou niekoľkých riadkov kódu? Použite rozhrania API na spracovanie dokumentov GroupDocs na úpravu, aktualizáciu a uloženie viac ako 30 formátov súborov."

############################# Header ############################
title: "Upraviť OTP v Java"
description: "Efektívne a robustné úpravy OTP pomocou GroupDocs.Editor na strane servera pre Java API bez použitia akéhokoľvek softvéru ako Microsoft alebo Open Office."
...
steps:
    enable: true
    title_left: "Kroky na úpravu OTP v Java"
    content_left: |
        [GroupDocs.Editor for Java](/sk/editor/java/) poskytuje vývojárom jednoduchý a priamy spôsob úpravy súborov OTP pomocou niekoľkých riadkov kódu.
        * Vytvorte inštanciu triedy `Editor` s povinnou cestou k súboru alebo bajtovým tokom a voliteľnou triedou `PresentationLoadOptions` a načítajte súbor OTP
        * Vytvorte a nastavte inštanciu triedy `PresentationEditOptions` pre formát súboru OTP
        * Zavolajte metódu `Editor.Edit()` a získajte dokument OTP vo formáte HTML, ktorý je možné ľahko upravovať pomocou ľubovoľného editora WYSIWYG.
        * Zavolajte metódu `Editor.Save()` a uložte upravený súbor OTP pomocou triedy `PresentationSaveOptions`
...
demos:
    enable: true
    title: "Živé ukážky editora OTP"
    content: |
        Upravte OTP hneď teraz na webovej stránke [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
