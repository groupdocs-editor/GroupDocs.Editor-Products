...
############################# Head ############################
head_title: "Edytor OTS — Edytuj OTS w Java"
head_description: "Jak edytować OTS w Java za pomocą kilku linijek kodu? Użyj interfejsów API przetwarzania dokumentów GroupDocs, aby edytować, aktualizować i zapisywać ponad 30 formatów plików."

############################# Header ############################
title: "Edytuj OTS w Java"
description: "Efektywna i solidna edycja OTS przy użyciu GroupDocs.Editor po stronie serwera dla interfejsów API Java, bez użycia jakiegokolwiek oprogramowania, takiego jak Microsoft lub Open Office."
...
steps:
    enable: true
    title_left: "Kroki edycji OTS w Java"
    content_left: |
        [GroupDocs.Editor for Java](/pl/editor/java/) zapewnia programistom łatwy i prosty sposób edytowania plików OTS przy użyciu kilku linijek kodu.
        * Utwórz instancję klasy `Editor` z obowiązkową ścieżką pliku lub strumieniem i opcjonalną klasą `SpreadsheetLoadOptions` i załaduj plik OTS
        * Utwórz i ustaw instancję klasy `SpreadsheetEditOptions` dla formatu pliku OTS
        * Wywołaj metodę `Editor.Edit()` i uzyskaj dokument OTS w formacie HTML, który można łatwo edytować dowolnym edytorem WYSIWYG.
        * Wywołaj metodę `Editor.Save()` i zapisz edytowany plik OTS przy użyciu klasy `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "OTS Demonstracje na żywo dla edytora"
    content: |
        Edytuj OTS już teraz, odwiedzając witrynę [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
