...
############################# Head ############################
head_title: "OTS Editor - Chỉnh sửa OTS trong Java"
head_description: "Làm cách nào để chỉnh sửa OTS trong Java bằng một vài dòng mã? Sử dụng API xử lý tài liệu GroupDocs để chỉnh sửa, cập nhật và lưu hơn 30 định dạng tệp."

############################# Header ############################
title: "Chỉnh sửa OTS trong Java"
description: "Chỉnh sửa OTS hiệu quả và mạnh mẽ bằng cách sử dụng GroupDocs.Editor phía máy chủ cho các API Java mà không cần sử dụng bất kỳ phần mềm nào như Microsoft hoặc Open Office."
...
steps:
    enable: true
    title_left: "Các bước chỉnh sửa OTS trong Java"
    content_left: |
        [GroupDocs.Editor for Java] (/vi / editor / java /) cung cấp một cách dễ dàng và đơn giản để nhà phát triển chỉnh sửa tệp OTS bằng một vài dòng mã.
        * Tạo một phiên bản của lớp `Editor` với đường dẫn hoặc luồng tệp bắt buộc và lớp` SpreadsheetLoadOptions` tùy chọn và tải tệp OTS
        * Tạo và đặt phiên bản lớp `SpreadsheetEditOptions` cho định dạng tệp OTS
        * Gọi phương thức `Editor.Edit ()` và lấy tài liệu OTS ở định dạng HTML có thể chỉnh sửa dễ dàng bằng bất kỳ trình soạn thảo WYSIWYG nào.
        * Gọi phương thức `Editor.Save ()` và lưu tệp OTS đã chỉnh sửa bằng cách sử dụng lớp `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "Bản trình diễn trực tiếp của OTS Editor"
    content: |
        Chỉnh sửa OTS ngay bây giờ bằng cách truy cập trang web [GroupDocs.Editor Live Demos] (https://products.groupdocs.app/editor/family) trang web.
