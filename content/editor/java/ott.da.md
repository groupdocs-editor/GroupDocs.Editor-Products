...
head_description: "Hvordan redigerer man OTT i Java ved hjælp af et par linjer kode? Brug GroupDocs-dokumenter, der behandler API'er til at redigere, opdatere og gemme mere end 30 filformater."

############################# Header ############################
title: "Rediger OTT i Java"
description: "Effektiv og robust OTT redigering ved hjælp af serverside GroupDocs.Editor til Java API'er, uden brug af software som Microsoft eller Open Office."
...
steps:
    enable: true
    title_left: "Trin til redigering af OTT i Java"
    content_left: |
        [GroupDocs.Editor for Java](/da/editor/java/) giver en nem og ligetil måde for udviklere at redigere OTT-filerne ved hjælp af et par linjer kode.
        * Opret en forekomst af klassen `Editor` med obligatorisk filsti eller stream og valgfri `WordProcessingLoadOptions` klasse og indlæs OTT filen
        * Opret og indstil klasseforekomsten `WordProcessingEditOptions` for filformatet OTT
        * Kald `Editor.Edit()`-metoden og få et OTT-dokument i HTML-format, der nemt kan redigeres med enhver WYSIWYG-editor.
        * Kald `Editor.Save()`-metoden og gem den redigerede OTT-fil ved hjælp af `WordProcessingSaveOptions`-klassen
...
demos:
    ...
    content: |
        Rediger OTT lige nu ved at besøge webstedet [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
