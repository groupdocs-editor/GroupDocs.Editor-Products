...
############################# Head ############################
head_title: "Éditeur POT : modifiez POT dans Java"
head_description: "Comment modifier POT dans Java en utilisant quelques lignes de code ? Utilisez les API de traitement de documents GroupDocs pour modifier, mettre à jour et enregistrer plus de 30 formats de fichiers."

############################# Header ############################
title: "Modifier POT dans Java"
description: "Édition POT efficace et robuste à l'aide de GroupDocs.Editor côté serveur pour les API Java, sans l'utilisation de logiciels tels que Microsoft ou Open Office."
...
steps:
    enable: true
    title_left: "Étapes pour modifier POT dans Java"
    content_left: |
        [GroupDocs.Editor for Java](/fr/editor/java/) offre aux développeurs un moyen simple et direct de modifier les fichiers POT à l'aide de quelques lignes de code.
        * Créez une instance de la classe `Editor` avec un chemin de fichier obligatoire ou un flux d'octets et une classe facultative `PresentationLoadOptions` et chargez le fichier POT
        * Créer et définir l'instance de classe `PresentationEditOptions` pour le format de fichier POT
        * Appelez la méthode `Editor.Edit()` et obtenez le document POT au format HTML facilement modifiable avec n'importe quel éditeur WYSIWYG.
        * Appelez la méthode `Editor.Save()` et enregistrez le fichier POT modifié à l'aide de la classe `PresentationSaveOptions`
...
demos:
    enable: true
    title: "POT démos en direct de l'éditeur"
    content: |
        Modifiez POT dès maintenant en visitant le site Web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
