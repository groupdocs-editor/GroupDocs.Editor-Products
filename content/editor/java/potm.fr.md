...
############################# Head ############################
head_title: "Éditeur POTM : modifiez POTM dans Java"
head_description: "Comment modifier POTM dans Java en utilisant quelques lignes de code ? Utilisez les API de traitement de documents GroupDocs pour modifier, mettre à jour et enregistrer plus de 30 formats de fichiers."

############################# Header ############################
title: "Modifier POTM dans Java"
description: "Édition POTM efficace et robuste à l'aide de GroupDocs.Editor côté serveur pour les API Java, sans l'utilisation de logiciels tels que Microsoft ou Open Office."
...
steps:
    enable: true
    title_left: "Étapes pour modifier POTM dans Java"
    content_left: |
        [GroupDocs.Editor for Java](/fr/editor/java/) offre aux développeurs un moyen simple et direct de modifier les fichiers POTM à l'aide de quelques lignes de code.
        * Créez une instance de la classe `Editor` avec un chemin de fichier obligatoire ou un flux d'octets et une classe facultative `PresentationLoadOptions` et chargez le fichier POTM
        * Créer et définir l'instance de classe `PresentationEditOptions` pour le format de fichier POTM
        * Appelez la méthode `Editor.Edit()` et obtenez le document POTM au format HTML facilement modifiable avec n'importe quel éditeur WYSIWYG.
        * Appelez la méthode `Editor.Save()` et enregistrez le fichier POTM modifié à l'aide de la classe `PresentationSaveOptions`
...
demos:
    enable: true
    title: "POTM démos en direct de l'éditeur"
    content: |
        Modifiez POTM dès maintenant en visitant le site Web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
