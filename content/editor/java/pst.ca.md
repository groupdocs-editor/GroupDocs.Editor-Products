...
############################# Head ############################
head_title: "Editor PST — Editeu PST a Java"
head_description: "Com editar PST a Java amb unes quantes línies de codi? Utilitzeu les API de processament de documents de GroupDocs per editar, actualitzar i desar més de 30 formats de fitxer."

############################# Header ############################
title: "Editeu PST a Java"
description: "Edició PST eficaç i robusta mitjançant GroupDocs.Editor del servidor per a les API Java, sense utilitzar cap programari com Microsoft o Open Office."
...
steps:
    enable: true
    title_left: "Passos per editar PST a Java"
    content_left: |
        [GroupDocs.Editor for Java](/ca/editor/java/) ofereix als desenvolupadors una manera senzilla i senzilla d'editar els fitxers PST utilitzant unes poques línies de codi.
        * Creeu una instància de la classe "Editor" amb el camí del fitxer o el flux de bytes obligatoris i carregueu el fitxer PST
        * Creeu i configureu la instància de classe `EmailEditOptions` per al format de fitxer PST
        * Truqueu al mètode `Editor.Edit()` i obteniu el document PST en format HTML que es pugui editar fàcilment amb qualsevol editor WYSIWYG.
        * Truqueu al mètode `Editor.Save()` i deseu el fitxer editat PST mitjançant la classe `EmailSaveOptions`
...
demos:
    enable: true
    title: "PST Editor de demostracions en directe"
    content: |
        Editeu PST ara mateix visitant el lloc web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
