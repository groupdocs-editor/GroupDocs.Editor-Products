...
############################# Head ############################
head_title: "Editor PST: modifica PST in Java"
head_description: "Come modificare PST in Java utilizzando poche righe di codice? Usa le API di elaborazione dei documenti di GroupDocs per modificare, aggiornare e salvare oltre 30 formati di file."

############################# Header ############################
title: "Modifica PST in Java"
description: "Modifica PST efficace e affidabile utilizzando GroupDocs.Editor lato server per API Java, senza l'uso di software come Microsoft o Open Office."
...
steps:
    enable: true
    title_left: "Passaggi per modificare PST in Java"
    content_left: |
        [GroupDocs.Editor for Java](/it/editor/java/) fornisce agli sviluppatori un modo semplice e diretto per modificare i file PST utilizzando poche righe di codice.
        * Crea un'istanza della classe `Editor` con percorso file obbligatorio o flusso di byte e carica il file PST
        * Crea e imposta l'istanza della classe `EmailEditOptions` per il formato file PST
        * Chiama il metodo `Editor.Edit()` e ottieni il documento PST in formato HTML facilmente modificabile con qualsiasi editor WYSIWYG.
        * Chiama il metodo `Editor.Save()` e salva il file PST modificato usando la classe `EmailSaveOptions`
...
demos:
    ...
    content: |
        Modifica PST in questo momento visitando il sito web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
