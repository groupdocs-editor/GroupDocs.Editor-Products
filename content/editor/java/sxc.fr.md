...
############################# Head ############################
head_title: "Éditeur SXC : modifiez SXC dans Java"
head_description: "Comment modifier SXC dans Java en utilisant quelques lignes de code ? Utilisez les API de traitement de documents GroupDocs pour modifier, mettre à jour et enregistrer plus de 30 formats de fichiers."

############################# Header ############################
title: "Modifier SXC dans Java"
description: "Édition SXC efficace et robuste à l'aide de GroupDocs.Editor côté serveur pour les API Java, sans l'utilisation de logiciels tels que Microsoft ou Open Office."
...
steps:
    enable: true
    title_left: "Étapes pour modifier SXC dans Java"
    content_left: |
        [GroupDocs.Editor for Java](/fr/editor/java/) offre aux développeurs un moyen simple et direct de modifier les fichiers SXC à l'aide de quelques lignes de code.
        * Créez une instance de la classe `Editor` avec un chemin de fichier ou un flux obligatoire et une classe facultative `SpreadsheetLoadOptions` et chargez le fichier SXC
        * Créez et définissez l'instance de classe `SpreadsheetEditOptions` pour le format de fichier SXC
        * Appelez la méthode `Editor.Edit()` et obtenez le document SXC au format HTML facilement modifiable avec n'importe quel éditeur WYSIWYG.
        * Appelez la méthode `Editor.Save()` et enregistrez le fichier SXC modifié à l'aide de la classe `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "SXC démos en direct de l'éditeur"
    content: |
        Modifiez SXC dès maintenant en visitant le site Web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
