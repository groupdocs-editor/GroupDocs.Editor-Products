...
############################# Head ############################
head_title: "Edytor SXC — Edytuj SXC w Java"
head_description: "Jak edytować SXC w Java za pomocą kilku linijek kodu? Użyj interfejsów API przetwarzania dokumentów GroupDocs, aby edytować, aktualizować i zapisywać ponad 30 formatów plików."

############################# Header ############################
title: "Edytuj SXC w Java"
description: "Efektywna i solidna edycja SXC przy użyciu GroupDocs.Editor po stronie serwera dla interfejsów API Java, bez użycia jakiegokolwiek oprogramowania, takiego jak Microsoft lub Open Office."
...
steps:
    enable: true
    title_left: "Kroki edycji SXC w Java"
    content_left: |
        [GroupDocs.Editor for Java](/pl/editor/java/) zapewnia programistom łatwy i prosty sposób edytowania plików SXC przy użyciu kilku linijek kodu.
        * Utwórz instancję klasy `Editor` z obowiązkową ścieżką pliku lub strumieniem i opcjonalną klasą `SpreadsheetLoadOptions` i załaduj plik SXC
        * Utwórz i ustaw instancję klasy `SpreadsheetEditOptions` dla formatu pliku SXC
        * Wywołaj metodę `Editor.Edit()` i uzyskaj dokument SXC w formacie HTML, który można łatwo edytować dowolnym edytorem WYSIWYG.
        * Wywołaj metodę `Editor.Save()` i zapisz edytowany plik SXC przy użyciu klasy `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "SXC Demonstracje na żywo dla edytora"
    content: |
        Edytuj SXC już teraz, odwiedzając witrynę [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
