...
############################# Head ############################
head_title: "TNEF Eagarthóir — Cuir TNEF in eagar in {{ PlatformName}}"
head_description: "Conas TNEF in Java a chur in eagar ag úsáid cúpla líne de chód? Bain úsáid as APIanna próiseála doiciméad GroupDocs chun 30+ formáid comhaid a chur in eagar, a nuashonrú agus a shábháil."

############################# Header ############################
title: "Cuir TNEF in eagar in {{ PlatformName}}"
description: "Eagarthóireacht éifeachtach agus láidir TNEF ag baint úsáide as taobh an fhreastalaí GroupDocs.Editor le haghaidh APIanna {{ PlatformName}}, gan úsáid a bhaint as aon bhogearraí ar nós Microsoft nó Open Office."
...
steps:
    enable: true
    title_left: "Céimeanna chun TNEF a chur in eagar in Java"
    content_left: |
        Soláthraíonn [GroupDocs.Editor for Java](/ga/editor/java/) bealach éasca agus simplí d’fhorbróirí chun na comhaid TNEF a chur in eagar ag úsáid cúpla líne cód.
        * Cruthaigh sampla de rang `Eagarthóir` le conair éigeantach comhaid nó sruth beart agus lódáil an comhad TNEF
        * Cruthaigh agus socraigh an sampla ranga `EmailEditOptions` don fhormáid comhaid TNEF
        * Glaoigh ar an modh `Editor.Edit()` agus faigh doiciméad TNEF i bhformáid HTML is furasta a chur in eagar le haon eagarthóir WYSIWYG.
        * Glaoigh ar an modh `Editor.Save()` agus sábháil an comhad TNEF atá curtha in eagar ag úsáid rang `EmailSaveOptions`
...
demos:
    enable: true
    title: "TNEF Eagarthóir Beo Taispeántais"
    content: |
        Cuir TNEF in eagar faoi láthair trí chuairt a thabhairt ar [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
