...
############################# Head ############################
head_title: "Edytor TNEF — Edytuj TNEF w Java"
head_description: "Jak edytować TNEF w Java za pomocą kilku linijek kodu? Użyj interfejsów API przetwarzania dokumentów GroupDocs, aby edytować, aktualizować i zapisywać ponad 30 formatów plików."

############################# Header ############################
title: "Edytuj TNEF w Java"
description: "Efektywna i solidna edycja TNEF przy użyciu GroupDocs.Editor po stronie serwera dla interfejsów API Java, bez użycia jakiegokolwiek oprogramowania, takiego jak Microsoft lub Open Office."
...
steps:
    enable: true
    title_left: "Kroki edycji TNEF w Java"
    content_left: |
        [GroupDocs.Editor for Java](/pl/editor/java/) zapewnia programistom łatwy i prosty sposób edytowania plików TNEF przy użyciu kilku linijek kodu.
        * Utwórz instancję klasy `Editor` z obowiązkową ścieżką pliku lub strumieniem bajtów i załaduj plik TNEF
        * Utwórz i ustaw instancję klasy `EmailEditOptions` dla formatu pliku TNEF
        * Wywołaj metodę `Editor.Edit()` i uzyskaj dokument TNEF w formacie HTML, który można łatwo edytować dowolnym edytorem WYSIWYG.
        * Wywołaj metodę `Editor.Save()` i zapisz edytowany plik TNEF przy użyciu klasy `EmailSaveOptions`
...
demos:
    enable: true
    title: "TNEF Demonstracje na żywo dla edytora"
    content: |
        Edytuj TNEF już teraz, odwiedzając witrynę [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
