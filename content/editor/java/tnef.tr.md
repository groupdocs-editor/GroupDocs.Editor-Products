...
############################# Head ############################
head_title: "TNEF Düzenleyici — TNEF öğesini Java içinde düzenleyin"
head_description: "TNEF, birkaç satır kod kullanarak Java içinde nasıl düzenlenir? 30'dan fazla dosya biçimini düzenlemek, güncellemek ve kaydetmek için API'leri işleyen GroupDocs belgelerini kullanın."

############################# Header ############################
title: "TNEF öğesini Java içinde düzenleyin"
description: "Microsoft veya Open Office gibi herhangi bir yazılım kullanmadan Java API'leri için sunucu tarafı GroupDocs.Editor kullanarak etkili ve sağlam TNEF düzenleme."
...
steps:
    enable: true
    title_left: "Java içinde TNEF Düzenleme Adımları"
    content_left: |
        [GroupDocs.Editor for Java](/tr/editor/java/), geliştiricilerin birkaç satır kod kullanarak TNEF dosyalarını düzenlemeleri için kolay ve anlaşılır bir yol sağlar.
        * Zorunlu dosya yolu veya bayt akışıyla bir "Editör" sınıfı örneği oluşturun ve TNEF dosyasını yükleyin
        * TNEF dosya biçimi için "EmailEditOptions" sınıf örneğini oluşturun ve ayarlayın
        * `Editor.Edit()` yöntemini çağırın ve herhangi bir WYSIWYG editörüyle kolayca düzenlenebilen HTML formatında TNEF belgesini alın.
        * `Editor.Save()` yöntemini çağırın ve `EmailSaveOptions` sınıfını kullanarak düzenlenmiş TNEF dosyasını kaydedin
...
demos:
    enable: true
    title: "TNEF Editör Canlı Demoları"
    content: |
        TNEF dosyasını hemen [GroupDocs.Editor Canlı Demoları](https://products.groupdocs.app/editor/family) web sitesini ziyaret ederek düzenleyin.
