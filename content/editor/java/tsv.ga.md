...
############################# Head ############################
head_title: "TSV Eagarthóir — Cuir TSV in eagar in {{ PlatformName}}"
head_description: "Conas TSV in Java a chur in eagar ag úsáid cúpla líne de chód? Bain úsáid as APIanna próiseála doiciméad GroupDocs chun 30+ formáid comhaid a chur in eagar, a nuashonrú agus a shábháil."

############################# Header ############################
title: "Cuir TSV in eagar in {{ PlatformName}}"
description: "Eagarthóireacht éifeachtach agus láidir TSV ag baint úsáide as taobh an fhreastalaí GroupDocs.Editor le haghaidh APIanna {{ PlatformName}}, gan úsáid a bhaint as aon bhogearraí ar nós Microsoft nó Open Office."
...
steps:
    enable: true
    title_left: "Céimeanna chun TSV a chur in eagar in Java"
    content_left: |
        Soláthraíonn [GroupDocs.Editor for Java](/ga/editor/java/) bealach éasca agus simplí d’fhorbróirí chun na comhaid TSV a chur in eagar ag úsáid cúpla líne cód.
        * Cruthaigh sampla de rang `Eagarthóir` le conair éigeantach comhaid nó sruth beart agus lódáil an comhad TSV
        * Cruthaigh an sampla ranga `DelimitedTextEditOptions` don fhormáid comhaid TSV agus sonraigh deighilteoir teaghrán éigeantach sa chruthaitheoir
        * Glaoigh ar an modh `Editor.Edit()` agus faigh doiciméad TSV i bhformáid HTML is furasta a chur in eagar le haon eagarthóir WYSIWYG
        * Glaoigh ar an modh `Editor.Save()` agus sábháil an comhad TSV atá curtha in eagar ag úsáid an ranga `DelimitedTextSaveOptions` agus an deighilteoir atá uait
...
demos:
    enable: true
    title: "TSV Eagarthóir Beo Taispeántais"
    content: |
        Cuir TSV in eagar faoi láthair trí chuairt a thabhairt ar [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
