...
############################# Head ############################
head_title: "Editor XLSM — Editeu XLSM a Java"
head_description: "Com editar XLSM a Java amb unes quantes línies de codi? Utilitzeu les API de processament de documents de GroupDocs per editar, actualitzar i desar més de 30 formats de fitxer."

############################# Header ############################
title: "Editeu XLSM a Java"
description: "Edició XLSM eficaç i robusta mitjançant GroupDocs.Editor del servidor per a les API Java, sense utilitzar cap programari com Microsoft o Open Office."
...
steps:
    enable: true
    title_left: "Passos per editar XLSM a Java"
    content_left: |
        [GroupDocs.Editor for Java](/ca/editor/java/) ofereix als desenvolupadors una manera senzilla i senzilla d'editar els fitxers XLSM utilitzant unes poques línies de codi.
        * Creeu una instància de la classe `Editor` amb la ruta o flux del fitxer obligatori i la classe `SpreadsheetLoadOptions` opcional i carregueu el fitxer XLSM
        * Creeu i configureu la instància de classe `SpreadsheetEditOptions` per al format de fitxer XLSM
        * Truqueu al mètode `Editor.Edit()` i obteniu el document XLSM en format HTML que es pugui editar fàcilment amb qualsevol editor WYSIWYG.
        * Truqueu al mètode `Editor.Save()` i deseu el fitxer editat XLSM mitjançant la classe `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "XLSM Editor de demostracions en directe"
    content: |
        Editeu XLSM ara mateix visitant el lloc web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
