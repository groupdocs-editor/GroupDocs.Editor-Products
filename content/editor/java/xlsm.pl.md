...
############################# Head ############################
head_title: "Edytor XLSM — Edytuj XLSM w Java"
head_description: "Jak edytować XLSM w Java za pomocą kilku linijek kodu? Użyj interfejsów API przetwarzania dokumentów GroupDocs, aby edytować, aktualizować i zapisywać ponad 30 formatów plików."

############################# Header ############################
title: "Edytuj XLSM w Java"
description: "Efektywna i solidna edycja XLSM przy użyciu GroupDocs.Editor po stronie serwera dla interfejsów API Java, bez użycia jakiegokolwiek oprogramowania, takiego jak Microsoft lub Open Office."
...
steps:
    enable: true
    title_left: "Kroki edycji XLSM w Java"
    content_left: |
        [GroupDocs.Editor for Java](/pl/editor/java/) zapewnia programistom łatwy i prosty sposób edytowania plików XLSM przy użyciu kilku linijek kodu.
        * Utwórz instancję klasy `Editor` z obowiązkową ścieżką pliku lub strumieniem i opcjonalną klasą `SpreadsheetLoadOptions` i załaduj plik XLSM
        * Utwórz i ustaw instancję klasy `SpreadsheetEditOptions` dla formatu pliku XLSM
        * Wywołaj metodę `Editor.Edit()` i uzyskaj dokument XLSM w formacie HTML, który można łatwo edytować dowolnym edytorem WYSIWYG.
        * Wywołaj metodę `Editor.Save()` i zapisz edytowany plik XLSM przy użyciu klasy `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "XLSM Demonstracje na żywo dla edytora"
    content: |
        Edytuj XLSM już teraz, odwiedzając witrynę [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
