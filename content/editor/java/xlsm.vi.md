...
############################# Head ############################
head_title: "XLSM Editor - Chỉnh sửa XLSM trong Java"
head_description: "Làm cách nào để chỉnh sửa XLSM trong Java bằng một vài dòng mã? Sử dụng API xử lý tài liệu GroupDocs để chỉnh sửa, cập nhật và lưu hơn 30 định dạng tệp."

############################# Header ############################
title: "Chỉnh sửa XLSM trong Java"
description: "Chỉnh sửa XLSM hiệu quả và mạnh mẽ bằng cách sử dụng GroupDocs.Editor phía máy chủ cho các API Java mà không cần sử dụng bất kỳ phần mềm nào như Microsoft hoặc Open Office."
...
steps:
    enable: true
    title_left: "Các bước chỉnh sửa XLSM trong Java"
    content_left: |
        [GroupDocs.Editor for Java] (/vi / editor / java /) cung cấp một cách dễ dàng và đơn giản để nhà phát triển chỉnh sửa tệp XLSM bằng một vài dòng mã.
        * Tạo một phiên bản của lớp `Editor` với đường dẫn hoặc luồng tệp bắt buộc và lớp` SpreadsheetLoadOptions` tùy chọn và tải tệp XLSM
        * Tạo và đặt phiên bản lớp `SpreadsheetEditOptions` cho định dạng tệp XLSM
        * Gọi phương thức `Editor.Edit ()` và lấy tài liệu XLSM ở định dạng HTML có thể chỉnh sửa dễ dàng bằng bất kỳ trình soạn thảo WYSIWYG nào.
        * Gọi phương thức `Editor.Save ()` và lưu tệp XLSM đã chỉnh sửa bằng cách sử dụng lớp `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "Bản trình diễn trực tiếp của XLSM Editor"
    content: |
        Chỉnh sửa XLSM ngay bây giờ bằng cách truy cập trang web [GroupDocs.Editor Live Demos] (https://products.groupdocs.app/editor/family) trang web.
