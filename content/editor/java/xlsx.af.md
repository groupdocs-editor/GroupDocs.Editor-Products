...
############################# Head ############################
head_title: "XLSX-redigeerder — Wysig XLSX in Java"
head_description: "Hoe om XLSX in Java te wysig deur 'n paar reëls kode te gebruik? Gebruik GroupDocs-dokumente wat API's verwerk om 30+ lêerformate te redigeer, op te dateer en te stoor."

############################# Header ############################
title: "Wysig XLSX in Java"
description: "Effektiewe en robuuste XLSX redigering deur bedienerkant GroupDocs.Editor vir Java API's te gebruik, sonder die gebruik van enige sagteware soos Microsoft of Open Office."
...
steps:
    enable: true
    title_left: "Stappe om XLSX in Java te wysig"
    content_left: |
        [GroupDocs.Editor for Java](/af/editor/java/) bied 'n maklike en reguit manier vir ontwikkelaars om die XLSX lêers te wysig deur 'n paar reëls kode te gebruik.
        * Skep 'n instansie van `Editor` klas met verpligte lêerpad of stroom en opsionele `SpreadsheetLoadOptions` klas en laai die XLSX lêer
        * Skep en stel die `SpreadsheetEditOptions`-klasinstansie vir die XLSX-lêerformaat
        * Roep `Editor.Edit()`-metode en verkry XLSX-dokument in HTML-formaat wat maklik met enige WYSIWYG-redigeerder geredigeer kan word.
        * Roep `Editor.Save()`-metode en stoor geredigeerde XLSX-lêer deur gebruik te maak van `SpreadsheetSaveOptions`-klas
...
demos:
    enable: true
    title: "XLSX Redigeerder Regstreekse Demo's"
    content: |
        Wysig XLSX op die oomblik deur [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family) webwerf te besoek.
