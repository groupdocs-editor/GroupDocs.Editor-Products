...
############################# Head ############################
head_title: "Editor XLSX — Editeu XLSX a Java"
head_description: "Com editar XLSX a Java amb unes quantes línies de codi? Utilitzeu les API de processament de documents de GroupDocs per editar, actualitzar i desar més de 30 formats de fitxer."

############################# Header ############################
title: "Editeu XLSX a Java"
description: "Edició XLSX eficaç i robusta mitjançant GroupDocs.Editor del servidor per a les API Java, sense utilitzar cap programari com Microsoft o Open Office."
...
steps:
    enable: true
    title_left: "Passos per editar XLSX a Java"
    content_left: |
        [GroupDocs.Editor for Java](/ca/editor/java/) ofereix als desenvolupadors una manera senzilla i senzilla d'editar els fitxers XLSX utilitzant unes poques línies de codi.
        * Creeu una instància de la classe `Editor` amb la ruta o flux del fitxer obligatori i la classe `SpreadsheetLoadOptions` opcional i carregueu el fitxer XLSX
        * Creeu i configureu la instància de classe `SpreadsheetEditOptions` per al format de fitxer XLSX
        * Truqueu al mètode `Editor.Edit()` i obteniu el document XLSX en format HTML que es pugui editar fàcilment amb qualsevol editor WYSIWYG.
        * Truqueu al mètode `Editor.Save()` i deseu el fitxer editat XLSX mitjançant la classe `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "XLSX Editor de demostracions en directe"
    content: |
        Editeu XLSX ara mateix visitant el lloc web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
