...
############################# Head ############################
head_title: "Editor XLSX – Upravte XLSX v Java"
head_description: "Ako upraviť XLSX v Java pomocou niekoľkých riadkov kódu? Použite rozhrania API na spracovanie dokumentov GroupDocs na úpravu, aktualizáciu a uloženie viac ako 30 formátov súborov."

############################# Header ############################
title: "Upraviť XLSX v Java"
description: "Efektívne a robustné úpravy XLSX pomocou GroupDocs.Editor na strane servera pre Java API bez použitia akéhokoľvek softvéru ako Microsoft alebo Open Office."
...
steps:
    enable: true
    title_left: "Kroky na úpravu XLSX v Java"
    content_left: |
        [GroupDocs.Editor for Java](/sk/editor/java/) poskytuje vývojárom jednoduchý a priamy spôsob úpravy súborov XLSX pomocou niekoľkých riadkov kódu.
        * Vytvorte inštanciu triedy `Editor` s povinnou cestou k súboru alebo streamu a voliteľnou triedou `SpreadsheetLoadOptions` a načítajte súbor XLSX
        * Vytvorte a nastavte inštanciu triedy `SpreadsheetEditOptions` pre formát súboru XLSX
        * Zavolajte metódu `Editor.Edit()` a získajte dokument XLSX vo formáte HTML, ktorý je možné ľahko upravovať pomocou ľubovoľného editora WYSIWYG.
        * Zavolajte metódu `Editor.Save()` a uložte upravený súbor XLSX pomocou triedy `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "Živé ukážky editora XLSX"
    content: |
        Upravte XLSX hneď teraz na webovej stránke [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
