...
############################# Head ############################
head_title: "Editor XLT — Editeu XLT a Java"
head_description: "Com editar XLT a Java amb unes quantes línies de codi? Utilitzeu les API de processament de documents de GroupDocs per editar, actualitzar i desar més de 30 formats de fitxer."

############################# Header ############################
title: "Editeu XLT a Java"
description: "Edició XLT eficaç i robusta mitjançant GroupDocs.Editor del servidor per a les API Java, sense utilitzar cap programari com Microsoft o Open Office."
...
steps:
    enable: true
    title_left: "Passos per editar XLT a Java"
    content_left: |
        [GroupDocs.Editor for Java](/ca/editor/java/) ofereix als desenvolupadors una manera senzilla i senzilla d'editar els fitxers XLT utilitzant unes poques línies de codi.
        * Creeu una instància de la classe `Editor` amb la ruta o flux del fitxer obligatori i la classe `SpreadsheetLoadOptions` opcional i carregueu el fitxer XLT
        * Creeu i configureu la instància de classe `SpreadsheetEditOptions` per al format de fitxer XLT
        * Truqueu al mètode `Editor.Edit()` i obteniu el document XLT en format HTML que es pugui editar fàcilment amb qualsevol editor WYSIWYG.
        * Truqueu al mètode `Editor.Save()` i deseu el fitxer editat XLT mitjançant la classe `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "XLT Editor de demostracions en directe"
    content: |
        Editeu XLT ara mateix visitant el lloc web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
