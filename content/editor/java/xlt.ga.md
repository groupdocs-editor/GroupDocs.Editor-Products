...
############################# Head ############################
head_title: "XLT Eagarthóir — Cuir XLT in eagar in {{ PlatformName}}"
head_description: "Conas XLT in Java a chur in eagar ag úsáid cúpla líne de chód? Bain úsáid as APIanna próiseála doiciméad GroupDocs chun 30+ formáid comhaid a chur in eagar, a nuashonrú agus a shábháil."

############################# Header ############################
title: "Cuir XLT in eagar in {{ PlatformName}}"
description: "Eagarthóireacht éifeachtach agus láidir XLT ag baint úsáide as taobh an fhreastalaí GroupDocs.Editor le haghaidh APIanna {{ PlatformName}}, gan úsáid a bhaint as aon bhogearraí ar nós Microsoft nó Open Office."
...
steps:
    enable: true
    title_left: "Céimeanna chun XLT a chur in eagar in Java"
    content_left: |
        Soláthraíonn [GroupDocs.Editor for Java](/ga/editor/java/) bealach éasca agus simplí d’fhorbróirí chun na comhaid XLT a chur in eagar ag úsáid cúpla líne cód.
        * Cruthaigh sampla de rang `Eagarthóir` le conair nó sruth comhaid éigeantach agus rang roghnach `SpreadsheetLoadOptions` agus lódáil an comhad XLT
        * Cruthaigh & socraigh an sampla ranga `SpreadsheetEditOptions` don fhormáid comhaid XLT
        * Glaoigh ar an modh `Editor.Edit()` agus faigh doiciméad XLT i bhformáid HTML is furasta a chur in eagar le haon eagarthóir WYSIWYG.
        * Glaoigh ar an modh `Editor.Save()` agus sábháil an comhad XLT atá curtha in eagar ag úsáid rang `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "XLT Eagarthóir Beo Taispeántais"
    content: |
        Cuir XLT in eagar faoi láthair trí chuairt a thabhairt ar [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
