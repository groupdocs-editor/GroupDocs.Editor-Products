...
############################# Head ############################
head_title: "XLTX Editor - Chỉnh sửa XLTX trong Java"
head_description: "Làm cách nào để chỉnh sửa XLTX trong Java bằng một vài dòng mã? Sử dụng API xử lý tài liệu GroupDocs để chỉnh sửa, cập nhật và lưu hơn 30 định dạng tệp."

############################# Header ############################
title: "Chỉnh sửa XLTX trong Java"
description: "Chỉnh sửa XLTX hiệu quả và mạnh mẽ bằng cách sử dụng GroupDocs.Editor phía máy chủ cho các API Java mà không cần sử dụng bất kỳ phần mềm nào như Microsoft hoặc Open Office."
...
steps:
    enable: true
    title_left: "Các bước chỉnh sửa XLTX trong Java"
    content_left: |
        [GroupDocs.Editor for Java] (/vi / editor / java /) cung cấp một cách dễ dàng và đơn giản để nhà phát triển chỉnh sửa tệp XLTX bằng một vài dòng mã.
        * Tạo một phiên bản của lớp `Editor` với đường dẫn hoặc luồng tệp bắt buộc và lớp` SpreadsheetLoadOptions` tùy chọn và tải tệp XLTX
        * Tạo và đặt phiên bản lớp `SpreadsheetEditOptions` cho định dạng tệp XLTX
        * Gọi phương thức `Editor.Edit ()` và lấy tài liệu XLTX ở định dạng HTML có thể chỉnh sửa dễ dàng bằng bất kỳ trình soạn thảo WYSIWYG nào.
        * Gọi phương thức `Editor.Save ()` và lưu tệp XLTX đã chỉnh sửa bằng cách sử dụng lớp `SpreadsheetSaveOptions`
...
demos:
    enable: true
    title: "Bản trình diễn trực tiếp của XLTX Editor"
    content: |
        Chỉnh sửa XLTX ngay bây giờ bằng cách truy cập trang web [GroupDocs.Editor Live Demos] (https://products.groupdocs.app/editor/family) trang web.
