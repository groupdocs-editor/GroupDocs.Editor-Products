...
############################# Head ############################
head_title: "Éditeur XML : modifiez XML dans Java"
head_description: "Comment modifier XML dans Java en utilisant quelques lignes de code ? Utilisez les API de traitement de documents GroupDocs pour modifier, mettre à jour et enregistrer plus de 30 formats de fichiers."

############################# Header ############################
title: "Modifier XML dans Java"
description: "Édition XML efficace et robuste à l'aide de GroupDocs.Editor côté serveur pour les API Java, sans l'utilisation de logiciels tels que Microsoft ou Open Office."
...
steps:
    enable: true
    title_left: "Étapes pour modifier XML dans Java"
    content_left: |
        [GroupDocs.Editor for Java](/fr/editor/java/) offre aux développeurs un moyen simple et direct de modifier les fichiers XML à l'aide de quelques lignes de code.
        * Créez une instance de la classe `Editor` avec un chemin de fichier obligatoire ou un flux d'octets et chargez le fichier XML
        * Créez et définissez l'instance de classe `TextEditOptions` pour le format de fichier XML
        * Appelez la méthode `Editor.Edit()` et obtenez le document XML au format HTML facilement modifiable avec n'importe quel éditeur WYSIWYG.
        * Appelez la méthode `Editor.Save()` et enregistrez le fichier XML modifié à l'aide de la classe `TextSaveOptions`
...
demos:
    enable: true
    title: "XML démos en direct de l'éditeur"
    content: |
        Modifiez XML dès maintenant en visitant le site Web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
