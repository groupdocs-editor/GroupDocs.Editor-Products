...
overview:
  enable: true
  title: "GroupDocs.Editor for .NET 한눈에"
  description: ".NET 애플리케이션에서 문서, 스프레드 시트, 슬라이드 및 전자 책을 편집, 변환 및 저장하는 API."
  features:
    # feature loop
    - title: "C#에서 여러 문서를 쉽게 편집합니다."
      content: "광범위한 형식을 지원하여 여러 PDF 및 사무실 파일을 원활하게 편집하십시오. GroupDocs.Editor for .NET 문서 편집을 빠르고 번거 로움없이 만듭니다."
...
platforms:
  enable: true
  title: "플랫폼 독립성"
  description: "다음에 따라 운영 체제, 프레임 워크 및 패키지 관리자가 지원됩니다"
  items:
    # platform loop
    - title: "Amazon"
      image: "amazon"
...
formats:
  ...
  title: "지원되는 파일 형식"
  description: |
    GroupDocs.Editor는 가져 오기 및 내보내기시 다음 파일 형식으로 작업을 지원합니다. ([전체 목록](https://docs.groupdocs.com/editor/net/supported-document-formats/)).
  groups:
    # group loop
    - color: "green"
      content: |
...
code_samples:
  enable: true
  title: "코드 샘플"
  description: "사용하는 일반적인 운영의 일부 사용 사례 GroupDocs.Editor for .NET"
  items:
    # code sample loop
    ...
    - title: "특정 Excel 워크 시트의 내용을 편집합니다"
      content: |
        ...
        {{< /landing/code >}}
    # code sample loop
    - title: "PDF에서 텍스트를 교체하십시오"
      content: |
        이 예제는 텍스트 내용을 다른 텍스트 내용을 교체하여 프로그래밍 방식으로 입력 PDF 파일의 내용을로드 및 편집하는 것을 보여줍니다. 그 후 수정 된 문서 컨텐츠는 새로운 PDF 문서로 저장됩니다.
        {{< landing/code title="텍스트를 교체하여 입력 PDF 편집하고 PDF로 다시 저장하십시오.">}}
        ```csharp {style=abap}
        
        // 경로별로 PDF 파일을로드하고 PDF로드 옵션을 지정합니다.
