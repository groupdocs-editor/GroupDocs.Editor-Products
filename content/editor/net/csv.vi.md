...
############################# Head ############################
head_title: "CSV Editor - Chỉnh sửa CSV trong C# .NET"
head_description: "Làm cách nào để chỉnh sửa CSV trong C# .NET bằng một vài dòng mã? Sử dụng API xử lý tài liệu GroupDocs để chỉnh sửa, cập nhật và lưu hơn 30 định dạng tệp."

############################# Header ############################
title: "Chỉnh sửa CSV trong C# .NET"
description: "Chỉnh sửa CSV hiệu quả và mạnh mẽ bằng cách sử dụng GroupDocs.Editor phía máy chủ cho các API C# .NET mà không cần sử dụng bất kỳ phần mềm nào như Microsoft hoặc Open Office."
...
steps:
    enable: true
    title_left: "Các bước chỉnh sửa CSV trong C#"
    content_left: |
        [GroupDocs.Editor for .NET] (/vi / editor / net /) cung cấp một cách dễ dàng và đơn giản để nhà phát triển chỉnh sửa tệp CSV bằng một vài dòng mã.
        * Tạo một phiên bản của lớp `Editor` với đường dẫn tệp bắt buộc hoặc luồng byte và tải tệp CSV
        * Tạo cá thể lớp `DelimitedTextEditOptions` cho định dạng tệp CSV và chỉ định dấu phân tách chuỗi bắt buộc trong hàm tạo
        * Gọi phương thức `Editor.Edit ()` và lấy tài liệu CSV ở định dạng HTML có thể dễ dàng chỉnh sửa bằng bất kỳ trình biên tập WYSIWYG nào
        * Gọi phương thức `Editor.Save ()` và lưu tệp CSV đã chỉnh sửa bằng cách sử dụng cá thể lớp `DelimitedTextSaveOptions` với dấu phân tách mong muốn
...
demos:
    enable: true
    title: "Bản trình diễn trực tiếp của CSV Editor"
    content: |
        Chỉnh sửa CSV ngay bây giờ bằng cách truy cập trang web [GroupDocs.Editor Live Demos] (https://products.groupdocs.app/editor/family) trang web.
