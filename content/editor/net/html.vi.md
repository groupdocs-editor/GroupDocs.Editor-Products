...
############################# Head ############################
head_title: "HTML Editor - Chỉnh sửa HTML trong C# .NET"
head_description: "Làm cách nào để chỉnh sửa HTML trong C# .NET bằng một vài dòng mã? Sử dụng API xử lý tài liệu GroupDocs để chỉnh sửa, cập nhật và lưu hơn 30 định dạng tệp."

############################# Header ############################
title: "Chỉnh sửa HTML trong C# .NET"
description: "Chỉnh sửa HTML hiệu quả và mạnh mẽ bằng cách sử dụng GroupDocs.Editor phía máy chủ cho các API C# .NET mà không cần sử dụng bất kỳ phần mềm nào như Microsoft hoặc Open Office."
...
steps:
    enable: true
    title_left: "Các bước chỉnh sửa HTML trong C#"
    content_left: |
        [GroupDocs.Editor for .NET] (/vi / editor / net /) cung cấp một cách dễ dàng và đơn giản để nhà phát triển chỉnh sửa tệp HTML bằng một vài dòng mã.
        * Tạo một phiên bản của lớp `Editor` với đường dẫn tệp bắt buộc hoặc luồng byte và tải tệp HTML
        * Tạo và đặt phiên bản lớp `TextEditOptions` cho định dạng tệp HTML
        * Gọi phương thức `Editor.Edit ()` và lấy tài liệu HTML ở định dạng HTML có thể chỉnh sửa dễ dàng bằng bất kỳ trình soạn thảo WYSIWYG nào.
        * Gọi phương thức `Editor.Save ()` và lưu tệp HTML đã chỉnh sửa bằng cách sử dụng lớp `TextSaveOptions`
...
demos:
    enable: true
    title: "Bản trình diễn trực tiếp của HTML Editor"
    content: |
        Chỉnh sửa HTML ngay bây giờ bằng cách truy cập trang web [GroupDocs.Editor Live Demos] (https://products.groupdocs.app/editor/family) trang web.
