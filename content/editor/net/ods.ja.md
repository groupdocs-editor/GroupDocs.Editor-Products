...
head_title: "ODSエディター– C＃.NETでODSを編集する"
head_description: "数行のコードを使用してC＃.NETでODSを編集するにはどうすればよいですか？ GroupDocsドキュメント処理APIを使用して、30以上のファイル形式を編集、更新、保存します."

title: "C＃でODSを編集する"
description: "MicrosoftやOpenOfficeなどのソフトウェアを使用せずに、サーバー側のGroupDocs.Editor for .NET APIを使用した効果的で堅牢なODS編集."
...
steps:
    enable: true
    title_left: "C＃でODSを編集する手順"
    content_left: |
        [GroupDocs.Editor](/editor/net/)は、開発者が数行のコードを使用してODSファイルを編集するための簡単で簡単な方法を提供します。

        * Editorクラスのインスタンスを作成し、フルパスでODSファイルをロードします
        *ODSファイルタイプのEditOptionsを作成および設定します
        * Editor.Editメソッドを呼び出し、WYSIWYGエディターで簡単に編集できるHTML形式のODSドキュメントを取得します。
        * Editor.Saveメソッドを呼び出し、編集したODSファイルを保存します
    ...
    code: |
        ...
        //入力ODSドキュメントを編集用に開きます—編集可能な中間ドキュメントを取得します
        EditableDocument beforeEdit = editor.Edit();

        //編集可能なドキュメントからODSドキュメントのコンテンツと関連リソースを取得します
        string content = beforeEdit.GetContent();

        //ODSドキュメントのコンテンツを何らかの方法で更新します
...
demos:
    ...
    content: |
        [GroupDocs.Editorライブデモ](https://products.groupdocs.app/editor/family)サイトにアクセスして、今すぐODSを編集してください。  
        ライブデモには次の利点があります
