...
############################# Head ############################
head_title: "ODT Editor – Rediger ODT i C# .NET"
head_description: "Hvordan redigere ODT i C# .NET ved å bruke noen få linjer med kode? Bruk GroupDocs-dokumenter som behandler APIer for å redigere, oppdatere og lagre 30+ filformater."

############################# Header ############################
title: "Rediger ODT i C# .NET"
description: "Effektiv og robust ODT-redigering ved bruk av GroupDocs.Editor på serversiden for C# .NET APIer, uten bruk av programvare som Microsoft eller Open Office."
...
steps:
    enable: true
    title_left: "Trinn for å redigere ODT i C#"
    content_left: |
        [GroupDocs.Editor for .NET](/no/editor/net/) gir utviklere en enkel og grei måte å redigere ODT-filene ved å bruke noen få linjer med kode.
        * Opprett en forekomst av 'Editor'-klassen med obligatorisk filbane eller strøm og valgfri 'WordProcessingLoadOptions'-klassen og last inn ODT-filen
        * Opprett og still inn klassen `WordProcessingEditOptions` for filformatet ODT
        * Kall `Editor.Edit()`-metoden og skaff ODT-dokumentet i HTML-format som enkelt kan redigeres med hvilken som helst WYSIWYG-editor.
        * Kall `Editor.Save()`-metoden og lagre den redigerte ODT-filen ved å bruke `WordProcessingSaveOptions`-klassen
...
demos:
    ...
    content: |
        Rediger ODT akkurat nå ved å gå til nettstedet [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
