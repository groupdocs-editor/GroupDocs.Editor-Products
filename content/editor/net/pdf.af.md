...
############################# Head ############################
head_title: "PDF-redigeerder — Wysig PDF in C# .NET"
head_description: "Hoe om PDF in C# .NET te wysig deur 'n paar reëls kode te gebruik? Gebruik GroupDocs-dokumente wat API's verwerk om 30+ lêerformate te redigeer, op te dateer en te stoor."

############################# Header ############################
title: "Wysig PDF in C# .NET"
description: "Effektiewe en robuuste PDF redigering deur bedienerkant GroupDocs.Editor vir C# .NET API's te gebruik, sonder die gebruik van enige sagteware soos Microsoft of Open Office."
...
steps:
    enable: true
    title_left: "Stappe om PDF in C# te wysig"
    content_left: |
        [GroupDocs.Editor for .NET](/af/editor/net/) bied 'n maklike en reguit manier vir ontwikkelaars om die PDF lêers te wysig deur 'n paar reëls kode te gebruik.
        * Skep 'n instansie van `Editor`-klas met verpligte lêerpad of stroom en opsionele `PdfLoadOptions`-klas vir PDF (nie van toepassing vir XPS nie) en laai die PDF-lêer
        * Skep en stel die `PdfEditOptions`-klasinstansie vir die PDF-lêerformaat
        * Roep `Editor.Edit()`-metode en verkry PDF-dokument in HTML-formaat wat maklik met enige WYSIWYG-redigeerder geredigeer kan word.
        * Roep `Editor.Save()`-metode en stoor geredigeerde PDF-lêer deur gebruik te maak van `PdfSaveOptions`-klas
...
demos:
    enable: true
    title: "PDF Redigeerder Regstreekse Demo's"
    content: |
        Wysig PDF op die oomblik deur [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family) webwerf te besoek.
