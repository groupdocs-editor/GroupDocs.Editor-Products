...
############################# Head ############################
head_title: "PDF Editor — Edit PDF in C# .NET"
head_description: "How to edit PDF in C# .NET using a few lines of code? Use GroupDocs documents processing APIs to edit, update and save 30+ file formats."

############################# Header ############################
title: "Edit PDF in C# .NET"
description: "Effective and robust PDF editing using server side GroupDocs.Editor for C# .NET APIs, without the use of any software like Microsoft or Open Office."
...
steps:
    enable: true
    title_left: "Steps to Edit PDF in C#"
    content_left: |
        [GroupDocs.Editor for .NET](/editor/net/) provides an easy and straightforward way for developers to edit the PDF files using a few lines of code.
        * Create an instance of `Editor` class with mandatory file path or stream and optional `PdfLoadOptions` class for PDF (not applicable for XPS) and load the PDF file
        * Create & set the `PdfEditOptions` class instance for the PDF file format
        * Call `Editor.Edit()` method and obtain PDF document in HTML format that is easily editable with any WYSIWYG-editor.
        * Call `Editor.Save()` method and save edited PDF file using `PdfSaveOptions` class
...
demos:
    ...
    content: |
        Edit PDF right now by visiting [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family) website.  
