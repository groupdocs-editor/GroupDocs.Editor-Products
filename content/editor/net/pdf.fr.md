...
############################# Head ############################
head_title: "Éditeur PDF : modifiez PDF dans C# .NET"
head_description: "Comment modifier PDF dans C# .NET en utilisant quelques lignes de code ? Utilisez les API de traitement de documents GroupDocs pour modifier, mettre à jour et enregistrer plus de 30 formats de fichiers."

############################# Header ############################
title: "Modifier PDF dans C# .NET"
description: "Édition PDF efficace et robuste à l'aide de GroupDocs.Editor côté serveur pour les API C# .NET, sans l'utilisation de logiciels tels que Microsoft ou Open Office."
...
steps:
    enable: true
    title_left: "Étapes pour modifier PDF dans C#"
    content_left: |
        [GroupDocs.Editor for .NET](/fr/editor/net/) offre aux développeurs un moyen simple et direct de modifier les fichiers PDF à l'aide de quelques lignes de code.
        * Créez une instance de la classe `Editor` avec un chemin de fichier ou un flux obligatoire et une classe facultative `PdfLoadOptions` pour PDF (non applicable pour XPS) et chargez le fichier PDF
        * Créez et définissez l'instance de classe `PdfEditOptions` pour le format de fichier PDF
        * Appelez la méthode `Editor.Edit()` et obtenez le document PDF au format HTML facilement modifiable avec n'importe quel éditeur WYSIWYG.
        * Appelez la méthode `Editor.Save()` et enregistrez le fichier PDF modifié à l'aide de la classe `PdfSaveOptions`
...
demos:
    enable: true
    title: "PDF démos en direct de l'éditeur"
    content: |
        Modifiez PDF dès maintenant en visitant le site Web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
