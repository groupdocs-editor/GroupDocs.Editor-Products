...
############################# Head ############################
head_title: "PDF Editor — Uredite PDF u C# .NET"
head_description: "Kako urediti PDF u C# .NET pomoću nekoliko redaka koda? Koristite API-je za obradu dokumenata GroupDocs za uređivanje, ažuriranje i spremanje više od 30 formata datoteka."

############################# Header ############################
title: "Uredi PDF u C# .NET"
description: "Učinkovito i robusno PDF uređivanje koristeći GroupDocs.Editor na strani poslužitelja za API-je C# .NET, bez upotrebe bilo kakvog softvera poput Microsofta ili Open Officea."
...
steps:
    enable: true
    title_left: "Koraci za uređivanje PDF u C#"
    content_left: |
        [GroupDocs.Editor for .NET](/hr/editor/net/) programerima pruža jednostavan i jednostavan način za uređivanje datoteka PDF pomoću nekoliko redaka koda.
        * Napravite instancu klase `Uređivač` s obaveznim putem datoteke ili streamom i izbornom klasom `PdfLoadOptions` za PDF (nije primjenjivo za XPS) i učitajte datoteku PDF
        * Stvorite i postavite instancu klase `PdfEditOptions` za format datoteke PDF
        * Pozovite metodu `Editor.Edit()` i dobijte PDF dokument u HTML formatu koji je lako uređivati ​​bilo kojim WYSIWYG uređivačem.
        * Pozovite metodu `Editor.Save()` i spremite uređenu datoteku PDF pomoću klase `PdfSaveOptions`
...
demos:
    enable: true
    title: "PDF Demonstracije uređivača uživo"
    content: |
        Uredite PDF odmah tako da posjetite [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family) web mjesto.
