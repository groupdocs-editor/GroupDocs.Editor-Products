...
############################# Head ############################
head_title: "PDF Editor — Edit PDF di C# .NET"
head_description: "Bagaimana cara mengedit PDF di C# .NET menggunakan beberapa baris kode? Gunakan API pemrosesan dokumen GroupDocs untuk mengedit, memperbarui, dan menyimpan 30+ format file."

############################# Header ############################
title: "Edit PDF di C# .NET"
description: "Pengeditan PDF yang efektif dan kuat menggunakan GroupDocs.Editor sisi server untuk API C# .NET, tanpa menggunakan perangkat lunak apa pun seperti Microsoft atau Open Office."
...
steps:
    enable: true
    title_left: "Langkah-langkah untuk Mengedit PDF di C#"
    content_left: |
        [GroupDocs.Editor for .NET](/id/editor/net/) menyediakan cara mudah dan langsung bagi pengembang untuk mengedit file PDF menggunakan beberapa baris kode.
        * Buat instance kelas `Editor` dengan jalur atau aliran file wajib dan kelas `PdfLoadOptions` opsional untuk PDF (tidak berlaku untuk XPS) dan muat file PDF
        * Buat & setel instance kelas `PdfEditOptions` untuk format file PDF
        * Panggil metode `Editor.Edit()` dan dapatkan dokumen PDF dalam format HTML yang mudah diedit dengan editor WYSIWYG apa pun.
        * Panggil metode `Editor.Save()` dan simpan file PDF yang telah diedit menggunakan kelas `PdfSaveOptions`
...
demos:
    ...
    content: |
        Edit PDF sekarang juga dengan mengunjungi situs web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
