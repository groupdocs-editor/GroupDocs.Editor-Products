...
############################# Head ############################
head_title: "Editor PDF: modifica PDF in C# .NET"
head_description: "Come modificare PDF in C# .NET utilizzando poche righe di codice? Usa le API di elaborazione dei documenti di GroupDocs per modificare, aggiornare e salvare oltre 30 formati di file."

############################# Header ############################
title: "Modifica PDF in C# .NET"
description: "Modifica PDF efficace e affidabile utilizzando GroupDocs.Editor lato server per API C# .NET, senza l'uso di software come Microsoft o Open Office."
...
steps:
    enable: true
    title_left: "Passaggi per modificare PDF in C#"
    content_left: |
        [GroupDocs.Editor for .NET](/it/editor/net/) fornisce agli sviluppatori un modo semplice e diretto per modificare i file PDF utilizzando poche righe di codice.
        * Crea un'istanza della classe `Editor` con percorso file o stream obbligatori e classe `PdfLoadOptions` opzionale per PDF (non applicabile per XPS) e carica il file PDF
        * Crea e imposta l'istanza della classe `PdfEditOptions` per il formato file PDF
        * Chiama il metodo `Editor.Edit()` e ottieni il documento PDF in formato HTML facilmente modificabile con qualsiasi editor WYSIWYG.
        * Chiama il metodo `Editor.Save()` e salva il file PDF modificato usando la classe `PdfSaveOptions`
...
demos:
    ...
    content: |
        Modifica PDF in questo momento visitando il sito web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
