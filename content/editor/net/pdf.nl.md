...
############################# Head ############################
head_title: "PDF Editor — Bewerk PDF in C# .NET"
head_description: "Hoe bewerk je PDF in C# .NET met een paar regels code? Gebruik API's voor het verwerken van documenten van GroupDocs om meer dan 30 bestandsindelingen te bewerken, bij te werken en op te slaan."

############################# Header ############################
title: "Bewerk PDF in C# .NET"
description: "Effectieve en robuuste PDF bewerking met Server-side GroupDocs.Editor voor C# .NET API's, zonder het gebruik van software zoals Microsoft of Open Office."
...
steps:
    enable: true
    title_left: "Stappen om PDF te bewerken in C#"
    content_left: |
        [GroupDocs.Editor for .NET](/nl/editor/net/) biedt ontwikkelaars een gemakkelijke en duidelijke manier om de PDF-bestanden te bewerken met een paar regels code.
        * Maak een instantie van de klasse 'Editor' met verplicht bestandspad of -stream en optionele klasse 'PdfLoadOptions' voor PDF (niet van toepassing op XPS) en laad het bestand PDF
        * Maak en stel de klasse-instantie 'PdfEditOptions' in voor de bestandsindeling PDF
        * Roep de `Editor.Edit()` methode aan en verkrijg een PDF document in HTML-formaat dat gemakkelijk kan worden bewerkt met elke WYSIWYG-editor.
        * Roep de `Editor.Save()`-methode aan en sla het bewerkte PDF-bestand op met de klasse `PdfSaveOptions`
...
demos:
    enable: true
    title: "PDF Editor Live demo's"
    content: |
        Bewerk PDF nu direct door naar de website [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family) te gaan.
