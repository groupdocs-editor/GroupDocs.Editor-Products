...
############################# Head ############################
head_title: "PDF Editor – Rediger PDF i C# .NET"
head_description: "Hvordan redigere PDF i C# .NET ved å bruke noen få linjer med kode? Bruk GroupDocs-dokumenter som behandler APIer for å redigere, oppdatere og lagre 30+ filformater."

############################# Header ############################
title: "Rediger PDF i C# .NET"
description: "Effektiv og robust PDF-redigering ved bruk av GroupDocs.Editor på serversiden for C# .NET APIer, uten bruk av programvare som Microsoft eller Open Office."
...
steps:
    enable: true
    title_left: "Trinn for å redigere PDF i C#"
    content_left: |
        [GroupDocs.Editor for .NET](/no/editor/net/) gir utviklere en enkel og grei måte å redigere PDF-filene ved å bruke noen få linjer med kode.
        * Opprett en forekomst av 'Editor'-klassen med obligatorisk filbane eller strøm og valgfri 'PdfLoadOptions'-klasse for PDF (ikke aktuelt for XPS) og last inn PDF-filen
        * Opprett og still inn klasseforekomsten `PdfEditOptions` for filformatet PDF
        * Kall `Editor.Edit()`-metoden og skaff PDF-dokumentet i HTML-format som enkelt kan redigeres med hvilken som helst WYSIWYG-editor.
        * Kall `Editor.Save()`-metoden og lagre den redigerte PDF-filen med `PdfSaveOptions`-klassen
...
demos:
    enable: true
    title: "PDF Editor Live Demoer"
    content: |
        Rediger PDF akkurat nå ved å gå til nettstedet [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
