...
############################# Head ############################
head_title: "Urejevalnik PDF — uredi PDF v C# .NET"
head_description: "Kako urediti PDF v C# .NET z uporabo nekaj vrstic kode? Uporabite API-je za obdelavo dokumentov GroupDocs za urejanje, posodabljanje in shranjevanje več kot 30 formatov datotek."

############################# Header ############################
title: "Uredi PDF v C# .NET"
description: "Učinkovito in robustno urejanje PDF z API-ji GroupDocs.Editor na strežniški strani za C# .NET, brez uporabe kakršne koli programske opreme, kot sta Microsoft ali Open Office."
...
steps:
    enable: true
    title_left: "Koraki za urejanje PDF v C#"
    content_left: |
        [GroupDocs.Editor for .NET](/sl/editor/net/) ponuja razvijalcem enostaven in preprost način za urejanje datotek PDF z uporabo nekaj vrstic kode.
        * Ustvarite primerek razreda `Urejevalnik` z obvezno potjo do datoteke ali tokom in izbirnim razredom `PdfLoadOptions` za PDF (ne velja za XPS) in naložite datoteko PDF
        * Ustvarite in nastavite primerek razreda `PdfEditOptions`` za obliko datoteke PDF
        * Pokličite metodo `Editor.Edit()` in pridobite dokument PDF v formatu HTML, ki ga je mogoče enostavno urejati s katerim koli urejevalnikom WYSIWYG.
        * Pokličite metodo `Editor.Save()` in shranite urejeno datoteko PDF z uporabo razreda `PdfSaveOptions`
...
demos:
    ...
    content: |
        Takoj uredite PDF tako, da obiščete spletno mesto [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
