...
############################# Head ############################
head_title: "PDF Editor - Chỉnh sửa PDF trong C# .NET"
head_description: "Làm cách nào để chỉnh sửa PDF trong C# .NET bằng một vài dòng mã? Sử dụng API xử lý tài liệu GroupDocs để chỉnh sửa, cập nhật và lưu hơn 30 định dạng tệp."

############################# Header ############################
title: "Chỉnh sửa PDF trong C# .NET"
description: "Chỉnh sửa PDF hiệu quả và mạnh mẽ bằng cách sử dụng GroupDocs.Editor phía máy chủ cho các API C# .NET mà không cần sử dụng bất kỳ phần mềm nào như Microsoft hoặc Open Office."
...
steps:
    enable: true
    title_left: "Các bước chỉnh sửa PDF trong C#"
    content_left: |
        [GroupDocs.Editor for .NET] (/vi / editor / net /) cung cấp một cách dễ dàng và đơn giản để nhà phát triển chỉnh sửa tệp PDF bằng một vài dòng mã.
        * Tạo một phiên bản của lớp `Editor` với đường dẫn hoặc luồng tệp bắt buộc và lớp` PdfLoadOptions` tùy chọn cho PDF (không áp dụng cho XPS) và tải tệp PDF
        * Tạo và đặt phiên bản lớp `Pdf EditOptions` cho định dạng tệp PDF
        * Gọi phương thức `Editor.Edit ()` và lấy tài liệu PDF ở định dạng HTML có thể chỉnh sửa dễ dàng bằng bất kỳ trình soạn thảo WYSIWYG nào.
        * Gọi phương thức `Editor.Save ()` và lưu tệp PDF đã chỉnh sửa bằng cách sử dụng lớp `Pdf SaveOptions`
...
demos:
    enable: true
    title: "Bản trình diễn trực tiếp của PDF Editor"
    content: |
        Chỉnh sửa PDF ngay bây giờ bằng cách truy cập trang web [GroupDocs.Editor Live Demos] (https://products.groupdocs.app/editor/family) trang web.
