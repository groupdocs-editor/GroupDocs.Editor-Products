...
############################# Head ############################
head_title: "PDF 编辑器 - 在 C# .NET 中编辑 PDF"
head_description: "如何使用几行代码在 C# .NET 中编辑 PDF？使用 GroupDocs 文档处理 API 编辑、更新和保存 30 多种文件格式。"

############################# Header ############################
title: "在 C# .NET 中编辑 PDF"
description: "使用用于 C# .NET API 的服务器端 GroupDocs.Editor 进行有效且强大的 PDF 编辑，无需使用 Microsoft 或 Open Office 等任何软件。"
...
steps:
    enable: true
    title_left: "在 C# 中编辑 PDF 的步骤"
    content_left: |
        [GroupDocs.Editor for .NET](/zh/editor/net/) 为开发人员提供了一种使用几行代码编辑 PDF 文件的简单直接的方法。
        * 创建一个具有强制性文件路径或流的“Editor”类实例以及 PDF 的可选“PdfLoadOptions”类（不适用于 XPS）并加载 PDF 文件
        * 为 PDF 文件格式创建和设置 `PdfEditOptions` 类实例
        * 调用 `Editor.Edit()` 方法并获得 HTML 格式的 PDF 文档，该文档可以使用任何所见即所得的编辑器轻松编辑。
        * 调用 `Editor.Save()` 方法并使用 `PdfSaveOptions` 类保存编辑的 PDF 文件
...
demos:
    ...
    title: "PDF 编辑器现场演示"
    content: |
        立即访问 [GroupDocs.Editor 现场演示](https://products.groupdocs.app/editor/family) 网站编辑 PDF。
        现场演示有以下好处
