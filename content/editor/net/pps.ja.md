...
head_title: "PPSエディター– C＃.NETでPPSを編集します"
head_description: "数行のコードを使用してC＃.NETでPPSを編集するにはどうすればよいですか？ GroupDocsドキュメント処理APIを使用して、30以上のファイル形式を編集、更新、保存します."

title: "C＃でPPSを編集する"
description: "MicrosoftやOpenOfficeなどのソフトウェアを使用せずに、サーバー側のGroupDocs.Editor for .NET APIを使用した効果的で堅牢なPPS編集."
...
steps:
    enable: true
    title_left: "C＃でPPSを編集する手順"
    content_left: |
        [GroupDocs.Editor](/editor/net/)は、開発者が数行のコードを使用してPPSファイルを編集するための簡単で簡単な方法を提供します。

        * Editorクラスのインスタンスを作成し、フルパスでPPSファイルをロードします
        *PPSファイルタイプのEditOptionsを作成および設定します
        * Editor.Editメソッドを呼び出し、WYSIWYGエディターで簡単に編集できるHTML形式のPPSドキュメントを取得します。
        * Editor.Saveメソッドを呼び出し、編集したPPSファイルを保存します
    ...
    code: |
        ...
        //入力PPSドキュメントを編集用に開きます—編集可能な中間ドキュメントを取得します
        EditableDocument beforeEdit = editor.Edit();

        //編集可能なドキュメントからPPSドキュメントのコンテンツと関連リソースを取得します
        string content = beforeEdit.GetContent();

        //PPSドキュメントのコンテンツを何らかの方法で更新します
...
demos:
    ...
    content: |
        [GroupDocs.Editorライブデモ](https://products.groupdocs.app/editor/family)サイトにアクセスして、今すぐPPSを編集してください。  
        ライブデモには次の利点があります
