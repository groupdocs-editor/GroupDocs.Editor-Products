...
############################# Head ############################
head_title: "PPTX エディタ — C# .NET の PPTX を編集"
head_description: "数行のコードを使用して C# .NET の PPTX を編集するには? GroupDocs ドキュメント処理 API を使用して、30 以上のファイル形式を編集、更新、保存します。"

############################# Header ############################
title: "C# .NETのPPTXを編集"
description: "Microsoft や Open Office などのソフトウェアを使用せずに、サーバー側の GroupDocs.Editor for C# .NET API を使用した効果的で堅牢な PPTX 編集。"
...
steps:
    enable: true
    title_left: "C# で PPTX を編集する手順"
    content_left: |
        [GroupDocs.Editor for .NET](/ja/editor/net/) は、開発者が数行のコードを使用して PPTX ファイルを編集する簡単で直接的な方法を提供します。
        * 必須のファイル パスまたはバイト ストリームとオプションの「PresentationLoadOptions」クラスを使用して「Editor」クラスのインスタンスを作成し、PPTX ファイルをロードします。
        * PPTX ファイル形式の `PresentationEditOptions` クラス インスタンスを作成して設定します
        * `Editor.Edit()` メソッドを呼び出して、WYSIWYG エディタで簡単に編集できる HTML 形式の PPTX ドキュメントを取得します。
        * `Editor.Save()` メソッドを呼び出し、`PresentationSaveOptions` クラスを使用して編集済みの PPTX ファイルを保存します
...
demos:
    enable: true
    title: "PPTXエディターのライブデモ"
    content: |
        [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family) Web サイトにアクセスして、今すぐ PPTX を編集してください。
        ライブデモには次の利点があります
