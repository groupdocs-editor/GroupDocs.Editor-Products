...
head_title: "SXCエディター– C＃.NETでSXCを編集します"
head_description: "数行のコードを使用してC＃.NETでSXCを編集するにはどうすればよいですか？ GroupDocsドキュメント処理APIを使用して、30以上のファイル形式を編集、更新、保存します."

title: "C＃でSXCを編集する"
description: "MicrosoftやOpenOfficeなどのソフトウェアを使用せずに、サーバー側のGroupDocs.Editor for .NET APIを使用した効果的で堅牢なSXC編集."
...
steps:
    enable: true
    title_left: "C＃でSXCを編集する手順"
    content_left: |
        [GroupDocs.Editor](/editor/net/)は、開発者が数行のコードを使用してSXCファイルを編集するための簡単で簡単な方法を提供します。

        * Editorクラスのインスタンスを作成し、フルパスでSXCファイルをロードします
        *SXCファイルタイプのEditOptionsを作成および設定します
        * Editor.Editメソッドを呼び出し、WYSIWYGエディターで簡単に編集できるHTML形式のSXCドキュメントを取得します。
        * Editor.Saveメソッドを呼び出し、編集したSXCファイルを保存します
    ...
    code: |
        ...
        //入力SXCドキュメントを編集用に開きます—編集可能な中間ドキュメントを取得します
        EditableDocument beforeEdit = editor.Edit();

        //編集可能なドキュメントからSXCドキュメントのコンテンツと関連リソースを取得します
        string content = beforeEdit.GetContent();

        //SXCドキュメントのコンテンツを何らかの方法で更新します
...
demos:
    enable: true
    title: "SXCエディターのライブデモ"
    content: |
        [GroupDocs.Editorライブデモ](https://products.groupdocs.app/editor/family)サイトにアクセスして、SXCを今すぐ編集してください。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-SXC"
          title: " SXCファイル形式について"
          content: |
            拡張子が.sxcのファイルは、StarOfficeCalcによって作成されたスプレッドシートです。
