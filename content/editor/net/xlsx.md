...
draft: false
otherformats: doc docx docm dotx xls xlsm ppt pptx pptm mobi epub html mhtml txt xml csv pdf xps

############################# Head ############################
head_title: "XLSX Editor — Edit XLSX in C# .NET"
head_description: "How to edit XLSX in C# .NET using a few lines of code? Use GroupDocs documents processing APIs to edit, update and save 30+ file formats."

############################# Header ############################
title: "Edit XLSX in C# .NET"
description: "Effective and robust XLSX editing using server side GroupDocs.Editor for C# .NET APIs, without the use of any software like Microsoft or Open Office."
...
steps:
    enable: true
    title_left: "Steps to Edit XLSX in C#"
    content_left: |
        [GroupDocs.Editor for .NET](/editor/net/) provides an easy and straightforward way for developers to edit the XLSX files using a few lines of code.
        * Create an instance of `Editor` class with mandatory file path or stream and optional `SpreadsheetLoadOptions` class and load the XLSX file
        * Create & set the `SpreadsheetEditOptions` class instance for the XLSX file format
        * Call `Editor.Edit()` method and obtain XLSX document in HTML format that is easily editable with any WYSIWYG-editor.
        * Call `Editor.Save()` method and save edited XLSX file using `SpreadsheetSaveOptions` class
