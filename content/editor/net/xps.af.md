...
############################# Head ############################
head_title: "XPS-redigeerder — Wysig XPS in C# .NET"
head_description: "Hoe om XPS in C# .NET te wysig deur 'n paar reëls kode te gebruik? Gebruik GroupDocs-dokumente wat API's verwerk om 30+ lêerformate te redigeer, op te dateer en te stoor."

############################# Header ############################
title: "Wysig XPS in C# .NET"
description: "Effektiewe en robuuste XPS redigering deur bedienerkant GroupDocs.Editor vir C# .NET API's te gebruik, sonder die gebruik van enige sagteware soos Microsoft of Open Office."
...
steps:
    enable: true
    title_left: "Stappe om XPS in C# te wysig"
    content_left: |
        [GroupDocs.Editor for .NET](/af/editor/net/) bied 'n maklike en reguit manier vir ontwikkelaars om die XPS lêers te wysig deur 'n paar reëls kode te gebruik.
        * Skep 'n instansie van `Editor`-klas met verpligte lêerpad of stroom en opsionele `PdfLoadOptions`-klas vir PDF (nie van toepassing vir XPS nie) en laai die XPS-lêer
        * Skep en stel die `XpsEditOptions`-klasinstansie vir die XPS-lêerformaat
        * Roep `Editor.Edit()`-metode en verkry XPS-dokument in HTML-formaat wat maklik met enige WYSIWYG-redigeerder geredigeer kan word.
        * Roep `Editor.Save()`-metode en stoor geredigeerde XPS-lêer deur gebruik te maak van `XpsSaveOptions`-klas
...
demos:
    enable: true
    title: "XPS Redigeerder Regstreekse Demo's"
    content: |
        Wysig XPS op die oomblik deur [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family) webwerf te besoek.
