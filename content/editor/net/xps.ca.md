...
############################# Head ############################
head_title: "Editor XPS — Editeu XPS a C# .NET"
head_description: "Com editar XPS a C# .NET amb unes quantes línies de codi? Utilitzeu les API de processament de documents de GroupDocs per editar, actualitzar i desar més de 30 formats de fitxer."

############################# Header ############################
title: "Editeu XPS a C# .NET"
description: "Edició XPS eficaç i robusta mitjançant GroupDocs.Editor del servidor per a les API C# .NET, sense utilitzar cap programari com Microsoft o Open Office."
...
steps:
    enable: true
    title_left: "Passos per editar XPS a C#"
    content_left: |
        [GroupDocs.Editor for .NET](/ca/editor/net/) ofereix als desenvolupadors una manera senzilla i senzilla d'editar els fitxers XPS utilitzant unes poques línies de codi.
        * Creeu una instància de la classe "Editor" amb la ruta o flux de fitxers obligatoris i la classe opcional "PdfLoadOptions" per a PDF (no aplicable a XPS) i carregueu el fitxer XPS
        * Creeu i configureu la instància de classe `XpsEditOptions` per al format de fitxer XPS
        * Truqueu al mètode `Editor.Edit()` i obteniu el document XPS en format HTML que es pugui editar fàcilment amb qualsevol editor WYSIWYG.
        * Truqueu al mètode `Editor.Save()` i deseu el fitxer XPS editat mitjançant la classe `XpsSaveOptions`
...
demos:
    enable: true
    title: "XPS Editor de demostracions en directe"
    content: |
        Editeu XPS ara mateix visitant el lloc web [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
