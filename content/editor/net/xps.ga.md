...
############################# Head ############################
head_title: "XPS Eagarthóir — Cuir XPS in eagar in {{ PlatformName}}"
head_description: "Conas XPS in C# .NET a chur in eagar ag úsáid cúpla líne de chód? Bain úsáid as APIanna próiseála doiciméad GroupDocs chun 30+ formáid comhaid a chur in eagar, a nuashonrú agus a shábháil."

############################# Header ############################
title: "Cuir XPS in eagar in {{ PlatformName}}"
description: "Eagarthóireacht éifeachtach agus láidir XPS ag baint úsáide as taobh an fhreastalaí GroupDocs.Editor le haghaidh APIanna {{ PlatformName}}, gan úsáid a bhaint as aon bhogearraí ar nós Microsoft nó Open Office."
...
steps:
    enable: true
    title_left: "Céimeanna chun XPS a chur in eagar in C#"
    content_left: |
        Soláthraíonn [GroupDocs.Editor for .NET](/ga/editor/net/) bealach éasca agus simplí d’fhorbróirí chun na comhaid XPS a chur in eagar ag úsáid cúpla líne cód.
        * Cruthaigh sampla de rang `Eagarthóir` le conair nó sruth comhaid éigeantach agus rang roghnach `PdfLoadOptions` le haghaidh PDF (neamhbhainteach le haghaidh XPS) agus luchtaigh an comhad XPS
        * Cruthaigh & socraigh an sampla ranga `XpsEditOptions` don fhormáid comhaid XPS
        * Glaoigh ar an modh `Editor.Edit()` agus faigh doiciméad XPS i bhformáid HTML is furasta a chur in eagar le haon eagarthóir WYSIWYG.
        * Cuir glaoch ar an modh `Editor.Save()` agus sábháil an comhad XPS atá curtha in eagar ag úsáid rang `XpsSaveOptions`
...
demos:
    enable: true
    title: "XPS Eagarthóir Beo Taispeántais"
    content: |
        Cuir XPS in eagar faoi láthair trí chuairt a thabhairt ar [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
