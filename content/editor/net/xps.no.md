...
head_description: "Hvordan redigere XPS i C# .NET ved å bruke noen få linjer med kode? Bruk GroupDocs-dokumenter som behandler APIer for å redigere, oppdatere og lagre 30+ filformater."

############################# Header ############################
title: "Rediger XPS i C# .NET"
description: "Effektiv og robust XPS-redigering ved bruk av GroupDocs.Editor på serversiden for C# .NET APIer, uten bruk av programvare som Microsoft eller Open Office."
...
steps:
    enable: true
    title_left: "Trinn for å redigere XPS i C#"
    content_left: |
        [GroupDocs.Editor for .NET](/no/editor/net/) gir utviklere en enkel og grei måte å redigere XPS-filene ved å bruke noen få linjer med kode.
        * Opprett en forekomst av 'Editor'-klassen med obligatorisk filbane eller strøm og valgfri 'PdfLoadOptions'-klasse for PDF (ikke aktuelt for XPS) og last inn XPS-filen
        * Opprett og still inn klasseforekomsten `XpsEditOptions` for filformatet XPS
        * Kall `Editor.Edit()`-metoden og skaff XPS-dokumentet i HTML-format som enkelt kan redigeres med hvilken som helst WYSIWYG-editor.
        * Kall `Editor.Save()`-metoden og lagre den redigerte XPS-filen med `XpsSaveOptions`-klassen
...
demos:
    ...
    content: |
        Rediger XPS akkurat nå ved å gå til nettstedet [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
