...
############################# Head ############################
head_title: "Editor XPS – Upravte XPS v C# .NET"
head_description: "Ako upraviť XPS v C# .NET pomocou niekoľkých riadkov kódu? Použite rozhrania API na spracovanie dokumentov GroupDocs na úpravu, aktualizáciu a uloženie viac ako 30 formátov súborov."

############################# Header ############################
title: "Upraviť XPS v C# .NET"
description: "Efektívne a robustné úpravy XPS pomocou GroupDocs.Editor na strane servera pre C# .NET API bez použitia akéhokoľvek softvéru ako Microsoft alebo Open Office."
...
steps:
    enable: true
    title_left: "Kroky na úpravu XPS v C#"
    content_left: |
        [GroupDocs.Editor for .NET](/sk/editor/net/) poskytuje vývojárom jednoduchý a priamy spôsob úpravy súborov XPS pomocou niekoľkých riadkov kódu.
        * Vytvorte inštanciu triedy `Editor` s povinnou cestou k súboru alebo streamu a voliteľnou triedou `PdfLoadOptions` pre PDF (neplatí pre XPS) a načítajte súbor XPS
        * Vytvorte a nastavte inštanciu triedy `XpsEditOptions` pre formát súboru XPS
        * Zavolajte metódu `Editor.Edit()` a získajte dokument XPS vo formáte HTML, ktorý je možné ľahko upravovať pomocou ľubovoľného editora WYSIWYG.
        * Zavolajte metódu `Editor.Save()` a uložte upravený súbor XPS pomocou triedy `XpsSaveOptions`
...
demos:
    enable: true
    title: "Živé ukážky editora XPS"
    content: |
        Upravte XPS hneď teraz na webovej stránke [GroupDocs.Editor Live Demos](https://products.groupdocs.app/editor/family).
