...
############################# Head ############################
head_title: "XPS Düzenleyici — XPS öğesini C# .NET içinde düzenleyin"
head_description: "XPS, birkaç satır kod kullanarak C# .NET içinde nasıl düzenlenir? 30'dan fazla dosya biçimini düzenlemek, güncellemek ve kaydetmek için API'leri işleyen GroupDocs belgelerini kullanın."

############################# Header ############################
title: "XPS öğesini C# .NET içinde düzenleyin"
description: "Microsoft veya Open Office gibi herhangi bir yazılım kullanmadan C# .NET API'leri için sunucu tarafı GroupDocs.Editor kullanarak etkili ve sağlam XPS düzenleme."
...
steps:
    enable: true
    title_left: "C# içinde XPS Düzenleme Adımları"
    content_left: |
        [GroupDocs.Editor for .NET](/tr/editor/net/), geliştiricilerin birkaç satır kod kullanarak XPS dosyalarını düzenlemeleri için kolay ve anlaşılır bir yol sağlar.
        * Zorunlu dosya yolu veya akışıyla ve PDF için isteğe bağlı 'PdfLoadOptions' sınıfıyla (XPS için geçerli değildir) bir "Editor" sınıfı örneği oluşturun ve XPS dosyasını yükleyin
        * XPS dosya biçimi için "XpsEditOptions" sınıf örneğini oluşturun ve ayarlayın
        * `Editor.Edit()` yöntemini çağırın ve herhangi bir WYSIWYG editörüyle kolayca düzenlenebilen HTML formatında XPS belgesini alın.
        * `Editor.Save()` yöntemini çağırın ve düzenlenen XPS dosyasını `XpsSaveOptions` sınıfını kullanarak kaydedin
...
demos:
    enable: true
    title: "XPS Editör Canlı Demoları"
    content: |
        XPS dosyasını hemen [GroupDocs.Editor Canlı Demoları](https://products.groupdocs.app/editor/family) web sitesini ziyaret ederek düzenleyin.
