...
############################# Head ############################
head_title: "XPS Editor - Chỉnh sửa XPS trong C# .NET"
head_description: "Làm cách nào để chỉnh sửa XPS trong C# .NET bằng một vài dòng mã? Sử dụng API xử lý tài liệu GroupDocs để chỉnh sửa, cập nhật và lưu hơn 30 định dạng tệp."

############################# Header ############################
title: "Chỉnh sửa XPS trong C# .NET"
description: "Chỉnh sửa XPS hiệu quả và mạnh mẽ bằng cách sử dụng GroupDocs.Editor phía máy chủ cho các API C# .NET mà không cần sử dụng bất kỳ phần mềm nào như Microsoft hoặc Open Office."
...
steps:
    enable: true
    title_left: "Các bước chỉnh sửa XPS trong C#"
    content_left: |
        [GroupDocs.Editor for .NET] (/vi / editor / net /) cung cấp một cách dễ dàng và đơn giản để nhà phát triển chỉnh sửa tệp XPS bằng một vài dòng mã.
        * Tạo một phiên bản của lớp `Editor` với đường dẫn hoặc luồng tệp bắt buộc và lớp` PdfLoadOptions` tùy chọn cho PDF (không áp dụng cho XPS) và tải tệp XPS
        * Tạo và đặt phiên bản lớp `Xps EditOptions` cho định dạng tệp XPS
        * Gọi phương thức `Editor.Edit ()` và lấy tài liệu XPS ở định dạng HTML có thể chỉnh sửa dễ dàng bằng bất kỳ trình soạn thảo WYSIWYG nào.
        * Gọi phương thức `Editor.Save ()` và lưu tệp XPS đã chỉnh sửa bằng cách sử dụng lớp `Xps SaveOptions`
...
demos:
    enable: true
    title: "Bản trình diễn trực tiếp của XPS Editor"
    content: |
        Chỉnh sửa XPS ngay bây giờ bằng cách truy cập trang web [GroupDocs.Editor Live Demos] (https://products.groupdocs.app/editor/family) trang web.
